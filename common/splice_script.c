...
static void dump_token_shallow(char **str, struct token *token, char *prefix)
{
	...
	if (!amount_sat_zero(token->amount_sat) || token->type == TOK_SATS) {
...
{
	if (token->type == TOK_SATS && amount_sat_zero(token->amount_sat))
		return false;
	...
	for (size_t i = 0; i < tal_count(input); i++) {
		...
		case TOK_SEGMENT:
			...
			if (lease) {
				...
				if (amount_sat_zero(lease->amount_sat))
					return new_error(ctx, LEASE_AMOUNT_ZERO,
							 lease->right,
							 "validate_and_clean");
		    ...
		    struct json_stream *js)
{
	...
	for (size_t i = 0; i < tal_count(splice); i++) {
		...
		if (!amount_sat_zero(splice[i]->lease_sat)) {
		...
		if (!amount_sat_zero(splice[i]->in_sat) || splice[i]->in_ppm) {
			json_object_start(js, "into_destination");
			if (!amount_sat_zero(splice[i]->in_sat))
				json_add_amount_sat_msat(js, "amount_msat",
							 splice[i]->in_sat);
			if (splice[i]->in_ppm)
		...
		if (!amount_sat_zero(splice[i]->out_sat)
			|| splice[i]->out_ppm) {
