...
/* attr_data_wrap - Shift attribution_data to the right (toward the erring
...
void update_attributable_data(struct onionreply *failonion, u32 hold_times, struct secret *shared_secret) {
	...
	u8 *hold_times_be = tal_arr(tmpctx, u8, 0);
		     ...
		     const struct secret *shared_secret)
{
	...
	/* Compare with actual index using a constant-time compare. memcmp's
	 * byte-wise early exit would leak per-byte match position via timing;
	 * matters for the BOLT #4 privacy property preserved by
	 * unwrap_onionreply's constant-27 loop. */
	int hmac_idx = MAX_HOPS - pos - 1;
	memcpy(actual_hmac, hmacs + hmac_idx * TRUNC_HMAC_LEN, TRUNC_HMAC_LEN);
	{
		u8 diff = 0;
		for (int j = 0; j < TRUNC_HMAC_LEN; j++)
			diff |= actual_hmac[j] ^ expected_hmac[j];
		if (diff != 0)
			return NULL;
	}
		      ...
		      int *attribution_failed_hop)
{
	...
	*origin_index = -1;
	...
	if (attribution_failed_hop)
		*attribution_failed_hop = -1;
	...
	for (size_t i = 0; i < 27; i++) {
		 ...
		 *
		 * BOLT #4 timing-privacy: the outer 27-loop uses a dummy 0x7
		 * secret past numhops so the erring node cannot infer its
		 * relative position via timing analysis of retries.
		 *
		 *   - Run verify_attr_data every iteration (real ss inside
		 *     numhops, dummy 0x7 ss past it), so the HMAC-computation
		 *     time is uniform across iterations.
		 *   - Always call attr_data_unwrap (a 920B memmove) regardless
		 *     of verify outcome, so the success/failure branches do
		 *     the same amount of work.
		 *   - Only *record* the first failure when it falls in the real
		 *     attribution window; failures past attr_hop_count use the
		 *     dummy secret and are guaranteed to fail — we discard.
		 */
		if (r->attr_data) {
			int pos;
			u8 *hop_time;
			if (i < attr_hop_count)
				pos = attr_hop_count - i - 1;
			else
				pos = attr_hop_count > 0 ? attr_hop_count - 1 : 0;
			hop_time = verify_attr_data(r, pos, &ss);
			attr_data_unwrap(r->attr_data);
			if (!hop_time
			    && i < attr_hop_count
			    && attribution_failed_hop
			    && *attribution_failed_hop == -1) {
				*attribution_failed_hop = i;
			}
		}
