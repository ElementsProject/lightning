...
fn test_coin_movement() {
    ...
    let u2: cln_rpc::notifications::CoinMovementNotification =
        serde_json::from_value(j2).unwrap();
    assert_eq!(u2.item_type, cln_rpc::notifications::CoinMovementType::CHAIN_MVT);
    assert_eq!(u2.primary_tag, Some(cln_rpc::notifications::CoinMovementPrimaryTag::WITHDRAWAL));
...
fn test_forward_event() {
    ...
    assert_eq!(u_failed.status, cln_rpc::notifications::ForwardEventStatus::LOCAL_FAILED);
...
fn test_warning() {
    ...
    let u_err: cln_rpc::notifications::WarningNotification =
        serde_json::from_value(j_err).unwrap();
...
fn test_pay_part_end() {
    ...
    assert_eq!(u_fail.payload.status, cln_rpc::notifications::PayPartEndPayloadStatus::FAILURE);
    assert!(u_fail.payload.failed_node_id.is_some());
    assert_eq!(u_fail.payload.error_code, Some(8195));
