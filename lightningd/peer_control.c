					   ...
					   unsigned int depth)
{
	...
	/* Reorg can change scid, so update scid when possible (depth=0 means
	 * the stale block with our funding tx was removed) */
	if (depth >= channel->minimum_depth || (channel->scid && depth)) {
		...
		if (!channel->scid)
			...
			/* Will restart channeld, initialized with updated scid, which
			 * should add it (at least our halve of the channel) to rtable. */
			channel_fail_transient(channel, "short_channel_id changed to %s (was %s)",
