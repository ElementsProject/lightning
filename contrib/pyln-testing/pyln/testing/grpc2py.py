...
def listpeers_peers_channels_htlcs2py(m):
    return remove_default({
        ...
        "state": camel2snake_case(primitives_pb2._HTLCSTATE.values_by_number[m.state].name),  # EnumField in generate_composite
    })


def listpeers_peers_channels2py(m):
    return remove_default({
        "state": camel2snake_case(primitives_pb2._CHANNELSTATE.values_by_number[m.state].name),  # EnumField in generate_composite
        "scratch_txid": hexlify(m.scratch_txid),  # PrimitiveField in generate_composite
