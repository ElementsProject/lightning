# A grpc model
from .model import ArrayField, F as Field, CompositeField, EnumField, PrimitiveField, Service
...
class GrpcGenerator:
    ...
    def gather_types(self, service):
        ...
        for method in service.methods:
            method.request.typename += "Request"
            method.response.typename += "Response"
    ...
    def generate_service(self, service: Service) -> None:
        ...
        for method in service.methods:
            self.write(
                f"	rpc {method.name}({method.request.typename}Request) returns ({method.response.typename}Response) {{}}\n",
                cleanup=False,
            )
    ...
    def generate_message(self, message: CompositeField):
        ...
        for i, f in enumerate(message.fields):
            if isinstance(f, ArrayField):
                ...
                if f.path in overrides:
                    typename = overrides[f.path]
                self.write(f"\t{typename} {f.normalized()} = {i+1};\n", False)
