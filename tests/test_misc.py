...
def test_signmessagewithkey(node_factory):
    ...
    with pytest.raises(RpcError, match=r'Address is not p2wpkh and it is not supported'):
        l1.rpc.signmessagewithkey(message, addr_p2tr)
    # refuse to sign if the address does not belong to us
    with pytest.raises(RpcError, match=r'Address is not found in the wallet\'s database'):
    ...
    assert subprocess.check_output(["devtools/bip137-checkmessage", message, response["signature"], response["pubkey"]]).decode("utf-8") == "Signature is valid!\n"
    # FIXME: check that response["pubkey"] produces address addr_bech32
