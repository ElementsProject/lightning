...
def test_names(node_factory, old_hsmsecret):
    if old_hsmsecret:
        ...
        configs = [
            ...
            (
                "033845802d25b4e074ccfd7cd8b339a41dc75bf9978a034800444b51d42b07799a",
                "SILENTGOPHER",
                "033845",
            ),
            (
            ...
            (
                "0258f3ff3e0853ccc09f6fe89823056d7c0c55c95fab97674df5e1ad97a72f6265",
                "BLUEFEED",
                "0258f3",
            ),
            (
                "02186115cb7e93e2cb4d9d9fe7a9cf5ff7a5784bfdda4f164ff041655e4bcd4fd0",
                "VIOLETYARD",
                "021861",
            ),
        ]
    ...
    for n, (key, alias, color) in zip(nodes, configs):
        assert n.daemon.is_in_log(
            r"public key {}, alias {}.* \(color #{}\)".format(key, alias, color)
        )


@unittest.skipIf(
    os.getenv("TEST_DB_PROVIDER", "sqlite3") != "sqlite3",
    "This migration is based on a sqlite3 snapshot",
)
def test_db_upgrade(node_factory):
    ...
    version = (
        subprocess.check_output(["lightningd/lightningd", "--version"])
        .decode("utf-8")
        .splitlines()[0]
    )
...
def test_bitcoin_failure(node_factory, bitcoind):
    ...
    l1 = node_factory.get_node(
        may_fail=True, broken_log=r"getrawblockbyheight|FATAL SIGNAL|backtrace"
    )
...
def test_bitcoin_pruned(node_factory, bitcoind):
    ...
    l1.daemon.wait_for_log(
        f"failed to fetch block {pruned_block} from the bitcoin backend"
    )
    ...
    l1.daemon.rpcproxy.mock_rpc(
        "getblockfrompeer", mock_getblockfrompeer(error=True, release_after=2)
    )
    bitcoind.generate_block(1)
    ...
    l1.daemon.rpcproxy.mock_rpc(
        "getblockfrompeer", mock_getblockfrompeer(error=True, release_after=10)
    )
    bitcoind.generate_block(1)
...
def test_lightningd_still_loading(node_factory, bitcoind, executor):
    ...
    l1, l2, l3 = node_factory.get_nodes(
        3,
        opts=[
            {"may_reconnect": True, "wait_for_bitcoind_sync": False},
            {"may_reconnect": True, "wait_for_bitcoind_sync": False},
            {},
        ],
    )
    ...
    assert l1.daemon.is_in_log(
        r"Sending HTLC while still syncing with bitcoin network \(104 vs 105\)"
    )
    ...
    if l1.config("experimental-dual-fund"):
        ...
        fut_open = executor.submit(
            l1.rpc.openchannel_init, l3.info["id"], "30000sat", psbt
        )
    else:
...
def test_htlc_sig_persistence(node_factory, bitcoind, executor, chainparams):
    ...
    l1 = node_factory.get_node(
        options={"dev-no-reconnect": None}, feerates=(7500, 7500, 7500, 7500)
    )
    ...
    _, txid, blocks = l1.wait_for_onchaind_tx(
        "OUR_HTLC_TIMEOUT_TO_US", "THEIR_UNILATERAL/OUR_HTLC"
    )
    assert blocks == 5
...
def test_htlc_out_timeout(node_factory, bitcoind, executor):
    ...
    assert (
        only_one(l2.rpc.listinvoices("test_htlc_out_timeout")["invoices"])["status"]
        == "unpaid"
    )
    ...
    l1.daemon.wait_for_log(
        "Offered HTLC 0 SENT_ADD_ACK_REVOCATION cltv .* hit deadline"
    )
    ...
    ((_, _, blocks1), (_, txid, blocks2)) = l1.wait_for_onchaind_txs(
    ...
    rawtx, txid, blocks = l1.wait_for_onchaind_tx(
        "OUR_DELAYED_RETURN_TO_WALLET", "OUR_HTLC_TIMEOUT_TX/DELAYED_OUTPUT_TO_US"
    )
    assert blocks == 4
    bitcoind.generate_block(4)

    # It should now claim both the to-local and htlc-timeout-tx outputs.
    l1.daemon.wait_for_logs(["sendrawtx exit 0.*{}".format(rawtx), "sendrawtx exit 0"])
...
def test_htlc_in_timeout(node_factory, bitcoind, executor):
    ...
    assert (
        only_one(l2.rpc.listinvoices("test_htlc_in_timeout")["invoices"])["status"]
        == "unpaid"
    )
    ...
    _, txid, blocks = l2.wait_for_onchaind_tx(
    ...
    rawtx, txid, blocks = l2.wait_for_onchaind_tx(
        "OUR_DELAYED_RETURN_TO_WALLET", "OUR_HTLC_SUCCESS_TX/DELAYED_OUTPUT_TO_US"
    )
    assert blocks == 4
    bitcoind.generate_block(4)
...
def test_p2tr_funding(node_factory, chainparams):
    ...
    def is_p2tr(output):
        return output["type"] == "witness_v1_taproot" and address == scriptpubkey_addr(
            output
        )
...
def test_withdraw_misc(node_factory, bitcoind, chainparams):
    ...
    with pytest.raises(RpcError):
        l1.rpc.withdraw(
            "xx1qrp33g0q5c5txsp9arysrx4k6zdkfs4nce4xj0gdcccefvpysxf3q0sl5k7", amount
        )
    with pytest.raises(RpcError):
        l1.rpc.withdraw(
            "tb1prp33g0q5c5txsp9arysrx4k6zdkfs4nce4xj0gdcccefvpysxf3qsm03tq", amount
        )
    with pytest.raises(RpcError):
        l1.rpc.withdraw(
            "tb1qrp33g0q5c5txsp9arysrx4k6zdkfs4nce4xj0gdcccefvpysxf3qxxxxxx", amount
        )
    out = l1.rpc.withdraw(waddr, amount)
    bitcoind.generate_block(1, wait_for_mempool=1)
    sync_blockheight(bitcoind, [l1])
    ...
    with pytest.raises(RpcError):
        l1.rpc.withdraw(
            "tb1qrp33g0q5c5txsp9arysrx4k6zdkfs4nce4xj0gdcccefvpysxf3q0sL5k7", amount
        )
    # Non-zero padding in 8-to-5 conversion
    with pytest.raises(RpcError):
        l1.rpc.withdraw(
            "tb1qrp33g0q5c5txsp9arysrx4k6zdkfs4nce4xj0gdcccefvpysxf3pjxtptv", amount
        )
...
def test_io_logging(node_factory, executor):
    ...
    assert not l1.daemon.is_in_log(
        r"channeld.*: \[OUT\] 0080", start=l1.daemon.logsearch_start
    )
    assert not l1.daemon.is_in_log(
        r"channeld.*: \[IN\] 0082", start=l1.daemon.logsearch_start
    )

    # IO logs should not appear in peer logs.
    peerlog = only_one(l2.rpc.listpeers(l1.info["id"], "io")["peers"])["log"]
    assert not any(l["type"] == "IO_OUT" or l["type"] == "IO_IN" for l in peerlog)
...
def test_address(node_factory):
    ...
    assert ret["address"] == {
        "type": "local socket",
        "socket": l1.daemon.opts["bind-addr"],
    }
...
def test_listconfigs(node_factory, bitcoind, chainparams):
    ...
    for deprecated in (True, False):
        l1 = node_factory.get_node(
            options={
                "log-prefix": "lightning1-xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx",
        ...
        for name, valfield, val in (
            ("allow-deprecated-apis", "value_bool", deprecated),
            ("network", "value_str", chainparams["name"]),
            ("ignore-fee-limits", "value_bool", False),
            (
                "log-prefix",
                "value_str",
                "lightning1-xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx",
            ),
        ):
            c = configs[name]
...
def test_cli(node_factory):
    ...
    lines = [
        ...
        if '"bolt11"' not in l and '"payment_hash"' not in l and '"expires_at"' not in l
    ]
    assert lines == [
        "{",
        '   "invoices": [',
    ...
    assert [l for l in lines if not re.search(r"^help\[[0-9]*\].", l)] == [
        "format-hint=simple"
    ]
...
def test_cli_multiline_help(node_factory):
    l1 = node_factory.get_node(
        options={"plugin": os.path.join(os.getcwd(), "tests/plugins/multiline-help.py")}
    )
...
def test_cli_commando(node_factory):
    ...
    l2.daemon.wait_for_log(
        r'jsonrpc#[0-9]*: "cli:help#[0-9]*/cln:commando#[0-9]*/commando:help#[0-9]*"\[IN\]'
    )
...
def test_daemon_option(node_factory):
    ...
    subprocess.run(l1.daemon.cmd_line, env=l1.daemon.env, check=True)
...
def test_blockchaintrack(node_factory, bitcoind):
    ...
    bitcoind.wait_for_log(
        r"InvalidChainFound: invalid block=.*  height={}".format(height + 1)
    )
...
def test_funding_reorg_private(node_factory, bitcoind):
    ...
    wait_for(
        lambda: only_one(l1.rpc.listpeerchannels()["channels"])["status"]
        == [
            "{}_AWAITING_LOCKIN:They've confirmed channel ready, we haven't yet.".format(
                daemon
            )
        ]
    )
...
def test_funding_reorg_remote_lags(node_factory, bitcoind):
    ...
    # We are OK with this!
    l2.daemon.wait_for_log(
        r"channel_gossip: received announcement sigs for 104x1x0 \(we have 103x1x0\)"
    )
    ...
    assert [
        c
        for c in l2.rpc.listpeerchannels()["channels"]
        if c["short_channel_id"] == "103x1x0"
    ] == []

    wait_for(
        lambda: [c["short_channel_id"] for c in l2.rpc.listchannels()["channels"]]
        == ["104x1x0"] * 2
    )
...
def test_funding_reorg_get_upset(node_factory, bitcoind):
    l1, l2 = node_factory.line_graph(
        ...
        opts=[
            {},
            {
                "broken_log": "Funding transaction has been reorged out in state CHANNELD_NORMAL"
            },
        ],
    )
    ...
    # l2 is upset!
    l2.daemon.wait_for_log(
        "Funding transaction has been reorged out in state CHANNELD_NORMAL"
    )
    assert (
        only_one(l2.rpc.listpeerchannels()["channels"])["state"]
        == "AWAITING_UNILATERAL"
    )
...
def test_recover(node_factory, bitcoind, old_hsmsecret):
    ...
    l1 = node_factory.get_node(
        start=False, options={"recover": recoverarg}, old_hsmsecret=old_hsmsecret
    )
    ...
    cmd_line = [
        "tools/lightning-hsmtool",
        "getsecret",
        os.path.join(l1.daemon.lightning_dir, TEST_NETWORK, "hsm_secret"),
    ]
    ...
    if old_hsmsecret:
        out = subprocess.run(
            cmd_line + ["lee"], stderr=subprocess.PIPE, timeout=TIMEOUT
        )
        assert "Invalid id: must be 4 characters" in out.stderr.decode("utf-8")
        assert out.returncode == 2

        out = subprocess.run(
            cmd_line + ["Leet"], stderr=subprocess.PIPE, timeout=TIMEOUT
        )
        ...
        for bad_bech32 in ["b", "o", "i", "1"]:
            out = subprocess.run(
                cmd_line + [bad_bech32 + "eet"], stderr=subprocess.PIPE, timeout=TIMEOUT
            )
            assert "Invalid id: must be valid bech32 string" in out.stderr.decode(
                "utf-8"
            )
            assert out.returncode == 2
...
def test_rescan(node_factory, bitcoind):
    ...
    # Will exit with failure code.
    assert l1.daemon.wait() == 1
    assert l1.daemon.is_in_stderr(
        r"bitcoind has gone backwards from 500000 to 105 blocks!"
    )
...
def test_bitcoind_goes_backwards(node_factory, bitcoind):
    ...
    assert not l1.daemon.is_in_log("Adding block 109", start=l1.daemon.logsearch_start)
...
def test_reserve_enforcement(node_factory, executor):
    ...
    l1, l2 = node_factory.line_graph(
        2, opts={"may_reconnect": True, "dev-no-reconnect": None, "allow_warning": True}
    )
...
def test_feerates(node_factory, anchors):
    opts = {"log-level": "io", "dev-no-fake-fees": True}
    if anchors is False:
    ...
    assert (
        feerates["warning_missing_feerates"]
    ...
    assert feerates["perkw"]["estimates"] == [
        {"blockcount": 2, "feerate": 15000, "smoothed_feerate": 15000}
    ]
    ...
    assert feerates["perkw"]["estimates"] == [
        {"blockcount": 2, "feerate": 15000, "smoothed_feerate": 15000},
        {"blockcount": 6, "feerate": 11000, "smoothed_feerate": 11000},
    ]
    ...
    assert feerates["perkb"]["estimates"] == [
        {"blockcount": 2, "feerate": 15000 * 4, "smoothed_feerate": 15000 * 4},
        {"blockcount": 6, "feerate": 11000 * 4, "smoothed_feerate": 11000 * 4},
        {"blockcount": 12, "feerate": 6250 * 4, "smoothed_feerate": 6250 * 4},
    ]
    ...
    assert feerates["perkw"]["estimates"] == [
        {"blockcount": 2, "feerate": 15000, "smoothed_feerate": 15000},
        {"blockcount": 6, "feerate": 11000, "smoothed_feerate": 11000},
        {"blockcount": 12, "feerate": 6250, "smoothed_feerate": 6250},
        {"blockcount": 100, "feerate": 5000, "smoothed_feerate": 5000},
    ]

    assert len(feerates["onchain_fee_estimates"]) == 6
    assert (
        feerates["onchain_fee_estimates"]["opening_channel_satoshis"]
        == feerates["perkw"]["opening"] * 702 // 1000
    )
    assert (
        feerates["onchain_fee_estimates"]["mutual_close_satoshis"]
        == feerates["perkw"]["mutual_close"] * 673 // 1000
    )
    if anchors:
        assert (
            feerates["onchain_fee_estimates"]["unilateral_close_satoshis"]
            == feerates["perkw"]["unilateral_anchor_close"] * 1112 // 1000
        )
    else:
        assert (
            feerates["onchain_fee_estimates"]["unilateral_close_satoshis"]
            == feerates["perkw"]["unilateral_close"] * 598 // 1000
        )
    assert (
    ...
    htlc_feerate = [
        f["feerate"] for f in feerates["perkw"]["estimates"] if f["blockcount"] == 6
    ][0]
...
def test_logging(node_factory):
    ...
    logpath_moved_2 = os.path.join(
        l1.daemon.lightning_dir, TEST_NETWORK, "logfile_moved_2"
    )
    ...
    # Multiple log files
    l2 = node_factory.get_node(
        options={"log-file": ["logfile1", "logfile2"]}, start=False
    )
    ...
    wait_for(
        lambda: os.path.exists(
            os.path.join(l2.daemon.lightning_dir, TEST_NETWORK, "lightning-rpc")
        )
    )
    lines = (
        subprocess.check_output(
            [
                "cli/lightning-cli",
                "--network={}".format(TEST_NETWORK),
                "--lightning-dir={}".format(l2.daemon.lightning_dir),
                "-H",
                "listconfigs",
            ]
        )
...
@unittest.skipIf(VALGRIND, "Valgrind sometimes fails assert on injected SEGV")
...
def test_configfile_before_chdir(node_factory):
    ...
    config = os.path.join(
        os.path.basename(l1.daemon.lightning_dir[:-1]), TEST_NETWORK, "test_configfile"
    )
    ...
    l1.start()
    assert l1.rpc.listconfigs()["configs"]["always-use-proxy"] == {
        "source": os.path.abspath(config) + ":1",
        "value_bool": True,
    }
    assert l1.rpc.listconfigs()["configs"]["proxy"] == {
        "source": os.path.abspath(config) + ":2",
        "value_str": "127.0.0.1:100",
    }
    os.chdir(olddir)
...
def test_json_error(node_factory):
    ...
    l1.rpc.check_request_schemas = False
    with pytest.raises(
        RpcError, match=r"id: should be a channel ID or short channel ID: invalid token"
    ):
        l1.rpc.close(
            {
                "tx": "",
                "txid": "2128c10f0355354479514f4a23eaa880d94e099406d419bbb0d800143accddbb",
                "channel_id": "bbddcc3a1400d8b0bb19d40694094ed980a8ea234a4f5179443555030fc12820",
            }
        )
...
def test_check_command(node_factory):
    ...
    sock.sendall(
        b'{"id":1, "jsonrpc":"2.0","method":"check","params":["help", "check"]}'
    )
    obj, _ = l1.rpc._readobj(sock, b"")
    assert obj["id"] == 1
    assert "result" in obj
    ...
    sock.sendall(
        b'{"id":1, "jsonrpc":"2.0","method":"check","params":["help", "a", "b"]}'
    )
    obj, _ = l1.rpc._readobj(sock, b"")
    assert obj["id"] == 1
    ...
    sock.sendall(
        b'{"id":1, "jsonrpc":"2.0","method":"check","params":["connect", "test", "x", "abcd"]}'
    )
    obj, _ = l1.rpc._readobj(sock, b"")
    assert obj["id"] == 1
    assert "result" not in obj
...
def test_bad_onion(node_factory, bitcoind):
    ...
    l1, l2, l3, l4 = node_factory.line_graph(
        4, wait_for_announce=True, opts={"log-level": "io"}
    )
...
def test_bad_onion_immediate_peer(node_factory, bitcoind):
    ...
    l1, l2 = node_factory.line_graph(
        2, opts=[{}, {"dev-fail-process-onionpacket": None}]
    )
    ...
    hops = [
        {
            ...
            "payload": serialize_payload_tlv(
                123000, 18 + 6, first_scid(l1, l2), blockheight
            ).hex(),
        },
        {
            "pubkey": l2.info["id"],
            "payload": serialize_payload_final_tlv(
                123000, 18, 123000, blockheight, inv["payment_secret"]
            ).hex(),
        },
    ]
...
def test_bitcoind_feerate_floor(node_factory, bitcoind, anchors):
    ...
    assert l1.rpc.feerates("perkb") == {
        "perkb": {
            ...
            "floor": 1012,
            "estimates": [
                {"blockcount": 2, "feerate": 60000, "smoothed_feerate": 60000},
                {"blockcount": 6, "feerate": 44000, "smoothed_feerate": 44000},
                {"blockcount": 12, "feerate": 30000, "smoothed_feerate": 30000},
                {"blockcount": 100, "feerate": 15000, "smoothed_feerate": 15000},
            ],
        },
    ...
    l1.daemon.rpcproxy.mock_rpc(
        "getmempoolinfo", {"mempoolminfee": 0.00010001, "minrelaytxfee": 0.00020001}
    )
    l1.restart()
    assert l1.rpc.feerates("perkb") == {
        "perkb": {
            ...
            "estimates": [
                ...
                {"blockcount": 6, "feerate": 44000, "smoothed_feerate": 44000},
                {"blockcount": 12, "feerate": 30000, "smoothed_feerate": 30000},
                {"blockcount": 100, "feerate": 20004, "smoothed_feerate": 20004},
            ],
        },
        "onchain_fee_estimates": {
            "opening_channel_satoshis": 5265,
    ...
    l1.daemon.rpcproxy.mock_rpc(
        "getmempoolinfo", {"mempoolminfee": 0.00030001, "minrelaytxfee": 0.00010001}
    )
    l1.restart()
    assert l1.rpc.feerates("perkb") == {
        "perkb": {
            ...
            "estimates": [
                ...
                {"blockcount": 6, "feerate": 44000, "smoothed_feerate": 44000},
                # This has increased (rounded up!)
                {"blockcount": 12, "feerate": 30004, "smoothed_feerate": 30004},
                # This has increased (rounded up!)
                {"blockcount": 100, "feerate": 30004, "smoothed_feerate": 30004},
            ],
        },
        "onchain_fee_estimates": {
            "opening_channel_satoshis": 5265,
...
def test_dev_demux(node_factory):
    ...
    assert (
        l1.rpc.call("dev", {"subcommand": "rhash", "secret": "00" * 32})["rhash"]
...
def test_list_features_only(node_factory):
    features = (
        subprocess.check_output(["lightningd/lightningd", "--list-features-only"])
        .decode("utf-8")
        .splitlines()
    )
    expected = [
...
def test_signmessage(node_factory):
    ...
    for c in corpus:
        ...
        assert subprocess.check_output(
            ["devtools/lightning-checkmessage", c[1], c[2]]
        ).decode("utf-8") == "Signature claims to be from key {}\n".format(c[3])

        subprocess.run(
            ["devtools/lightning-checkmessage", c[1], c[2], c[3]], check=True
        )

        with pytest.raises(subprocess.CalledProcessError):
            subprocess.run(
                ["devtools/lightning-checkmessage", c[1] + "modified", c[2], c[3]],
                check=True,
            )
...
def test_include(node_factory):
    ...
    assert l1.rpc.listconfigs("alias")["configs"]["alias"] == {
        "source": os.path.join(subdir, "conf2") + ":1",
        "value_str": "conf2",
    }
...
def test_config_in_subdir(node_factory, chainparams):
    ...
    assert l1.rpc.listconfigs("alias")["configs"]["alias"] == {
        "source": os.path.join(subdir, "config") + ":1",
        "value_str": "test_config_in_subdir",
    }
    ...
    assert out.returncode == 1
    assert "network={}: not permitted in network-specific configuration files".format(
        network
    ...
    assert out.returncode == 1
    assert (
        "lightning-dir={}/test: not permitted in implicit configuration files".format(
            l1.daemon.opts.get("lightning-dir")
        )
        in out.stderr.decode("utf-8")
    )
...
def restore_valgrind(node, subdir):
    ...
    for f in os.listdir(subdir):
        if f.startswith("valgrind-errors."):
            shutil.move(os.path.join(subdir, f), node.daemon.opts.get("lightning-dir"))
...
def test_testnet_upgrade(node_factory):
    ...
    # Make it old-style
    os.rename(
        os.path.join(basedir, TEST_NETWORK, "hsm_secret"),
    ...
    assert not l1.daemon.is_in_log(
        "Moving lightningd-testnet.pid into {}/".format(netdir)
    )
...
def test_regtest_upgrade(node_factory):
    ...
    assert not os.path.isfile(
        os.path.join(netdir, "lightningd-{}.pid".format(TEST_NETWORK))
    )
    assert os.path.isfile(
        os.path.join(basedir, "lightningd-{}.pid".format(TEST_NETWORK))
    )
    assert not os.path.isfile(os.path.join(netdir, "config"))
    assert os.path.isfile(os.path.join(basedir, "config"))
...
@unittest.skipIf(
    VALGRIND, "Testing pyln doesn't exercise anything interesting in the c code."
)
def test_unix_socket_path_length(
...
def test_custommsg_triggers_notification(node_factory):
    ...
    plugin_path = os.path.join(
        os.path.dirname(__file__), "plugins", "custommsg_notification.py"
    )
    ...
    l1.connect(l2)
    wait_for(
        lambda: [p["connected"] for p in l2.rpc.listpeers(l1.info["id"])["peers"]]
        == [True]
    )
...
def test_recoverchannel(node_factory):
    ...
    assert len(stubs) == 1
    assert (
        stubs[0] == "c3a7b9d74a174497122bc52d74d6d69836acadc77e0429c6d8b68b48d5c9139a"
    )
...
def test_getemergencyrecoverdata(node_factory):
    ...
    with open(
        os.path.join(l1.daemon.lightning_dir, TEST_NETWORK, "emergency.recover"), "rb"
    ) as f:
        lines = f.read().hex()
...
def test_emergencyrecover_old_format_handling(node_factory, bitcoind):
    ...
    with open(
        os.path.join(l1.daemon.lightning_dir, TEST_NETWORK, "emergency.recover"), "wb"
    ) as f:
    ...
    assert len(stubs) == 1
    assert (
        stubs[0] == "3497625a774a5e1839f1a4a6b23a6a06493817ae90ff4ed0a536f4202845de2f"
    )
    assert l1.daemon.is_in_log(
        "Watching for funding txid: 2fde452820f436a5d04eff90ae173849066a3ab2a6a4f139185e4a775a629734"
    )
...
def test_emergencyrecoverpenaltytxn(node_factory, bitcoind):
    ...
    l1.daemon.wait_for_log(
        "Sending a bogus channel_reestablish message to make the peer unilaterally close the channel."
    )
...
def test_recover_plugin(node_factory, bitcoind):
    ...
    l2.daemon.wait_for_log(
        f"{l1.info['id']}-chan#1: State changed from FUNDING_SPEND_SEEN to ONCHAIN"
    )
    wait_for(lambda: l2.rpc.listfunds()["channels"][0]["state"] == "ONCHAIN")
...
def test_restorefrompeer(node_factory, bitcoind):
    ...
    l1.rpc.connect(l2.info["id"], "localhost", l2.port)
    l1.daemon.wait_for_logs(["peer_out WIRE_PEER_STORAGE", "peer_in WIRE_PEER_STORAGE"])
    l2.daemon.wait_for_logs(["peer_out WIRE_PEER_STORAGE", "peer_in WIRE_PEER_STORAGE"])
...
def test_commitfee_option(node_factory):
    ...
    l1, l2 = node_factory.get_nodes(
        2, opts=[{"commit-fee": "200", "start": False}, {"start": False}]
    )
...
def test_listtransactions(node_factory):
    ...
    txids = [
        i["txid"]
        for tx in l1.rpc.listtransactions()["transactions"]
        for i in tx["inputs"]
    ]
...
def test_listforwards_and_listhtlcs(node_factory, bitcoind):
    ...
    assert (
        len(l2.rpc.listhtlcs(id=c12, index="updated", start=1, limit=1)["htlcs"]) == 1
    )
    ...
    with pytest.raises(RpcError):
        l1.rpc.sendpay(
            failed_route,
            failed_inv["payment_hash"],
            payment_secret=failed_inv["payment_secret"],
        )
    ...
    assert [h["amount_msat"] for h in c1htlcs] == [
        Millisatoshi(1001),
        Millisatoshi(2001),
        Millisatoshi(4001),
    ]
    ...
    assert (
        l2.rpc.listhtlcs(index="created", start=3, limit=100)["htlcs"] == allhtlcs[2:]
    )
    assert l2.rpc.listhtlcs(index="created", start=3, limit=1)["htlcs"] == [allhtlcs[2]]
    assert l2.rpc.listhtlcs(id=c12, index="created", start=1)["htlcs"] == [
        allhtlcs[0],
        allhtlcs[2],
        allhtlcs[4],
    ]
    ...
    assert l2.rpc.listhtlcs(id=c12, index="created", start=2, limit=1)["htlcs"] == [
        allhtlcs[2]
    ]
    assert l2.rpc.listhtlcs(id=c12, index="created", start=3, limit=2)["htlcs"] == [
        allhtlcs[2],
        allhtlcs[4],
    ]
    ...
    assert (
        l2.rpc.listhtlcs(
            index="updated", start=updatedhtlcs[0]["updated_index"] + 1, limit=100
        )["htlcs"]
        == updatedhtlcs[1:]
    )
    assert l2.rpc.listhtlcs(
        index="updated", start=updatedhtlcs[1]["updated_index"] + 1, limit=1
    )["htlcs"] == [updatedhtlcs[2]]
    c12htlcs = [h for h in updatedhtlcs if h["short_channel_id"] == c12]
    assert (
        l2.rpc.listhtlcs(id=c12, index="updated", start=c12htlcs[0]["updated_index"])[
            "htlcs"
        ]
        == c12htlcs
    )
    assert l2.rpc.listhtlcs(
        id=c12, index="updated", start=c12htlcs[0]["updated_index"] + 1
    )["htlcs"] == [c12htlcs[1], c12htlcs[2]]
    assert l2.rpc.listhtlcs(
        id=c12, index="updated", start=c12htlcs[2]["updated_index"], limit=1
    )["htlcs"] == [c12htlcs[2]]
    assert l2.rpc.listhtlcs(
        id=c12, index="updated", start=c12htlcs[1]["updated_index"], limit=2
    )["htlcs"] == [c12htlcs[1], c12htlcs[2]]
...
def test_listforwards_wait(node_factory, executor):
    ...
    assert waitres == {"subsystem": "forwards", "created": 0}

    # Now ask for 1.
    waitcreate = executor.submit(
        l2.rpc.wait, subsystem="forwards", indexname="created", nextvalue=1
    )
    waitupdate = executor.submit(
        l2.rpc.wait, subsystem="forwards", indexname="updated", nextvalue=1
    )
    l2.daemon.wait_for_logs(
        ["waiting on forwards created 1", "waiting on forwards updated 1"]
    )
    ...
    waitcreate = executor.submit(
        l2.rpc.wait, subsystem="forwards", indexname="created", nextvalue=2
    )
    waitupdate = executor.submit(
        l2.rpc.wait, subsystem="forwards", indexname="updated", nextvalue=2
    )
    l2.daemon.wait_for_logs(
        ["waiting on forwards created 2", "waiting on forwards updated 2"]
    )
    time.sleep(1)
    ...
    assert [
        (p["created_index"], p["in_msat"], p["status"])
        for p in l2.rpc.listforwards(index="created")["forwards"]
    ] == [(1, Millisatoshi(amt1 + 1), "settled"), (2, Millisatoshi(amt2 + 1), "failed")]
    assert [
        (p["created_index"], p["in_msat"], p["status"])
        for p in l2.rpc.listforwards(index="created", start=2)["forwards"]
    ] == [(2, Millisatoshi(amt2 + 1), "failed")]
    assert [
        (p["created_index"], p["in_msat"], p["status"])
        for p in l2.rpc.listforwards(index="created", limit=1)["forwards"]
    ] == [(1, Millisatoshi(amt1 + 1), "settled")]

    # We can also filter by status.
    assert [
        (p["created_index"], p["in_msat"], p["status"])
        for p in l2.rpc.listforwards(status="failed", index="created", limit=2)[
            "forwards"
        ]
    ] == [(2, Millisatoshi(amt2 + 1), "failed")]
    ...
    assert [
        (p["created_index"], p["in_msat"], p["status"])
        for p in l2.rpc.listforwards(status="failed", index="updated", limit=2)[
            "forwards"
        ]
    ] == [(2, Millisatoshi(amt2 + 1), "failed")]

    # Finally, check deletion.
    waitfut = executor.submit(
        l2.rpc.wait, subsystem="forwards", indexname="deleted", nextvalue=1
    )
    ...
    assert waitres == {
        "subsystem": "forwards",
        "deleted": 1,
        "forwards": {"in_channel": scid12, "in_htlc_id": 1, "status": "failed"},
    }
...
def test_listhtlcs_wait(node_factory, bitcoind, executor):
    ...
    assert waitres == {"subsystem": "htlcs", "created": 0}

    # Now ask for 1.
    waitcreate = executor.submit(
        l2.rpc.wait, subsystem="htlcs", indexname="created", nextvalue=1
    )
    waitupdate = executor.submit(
        l2.rpc.wait, subsystem="htlcs", indexname="updated", nextvalue=1
    )
    l2.daemon.wait_for_logs(
        ["waiting on htlcs created 1", "waiting on htlcs updated 1"]
    )
    ...
    assert l2.rpc.wait(subsystem="htlcs", indexname="created", nextvalue=2) == {
        "created": 2,
        "subsystem": "htlcs",
    }
    ...
    waitcreate = executor.submit(
        l2.rpc.wait, subsystem="htlcs", indexname="created", nextvalue=4
    )
    ...
    waitfut = executor.submit(
        l2.rpc.wait, subsystem="htlcs", indexname="deleted", nextvalue=1
    )
...
def test_version_reexec(node_factory, bitcoind):
    ...
    l1, l2 = node_factory.get_nodes(
        ...
        opts=[
            {
                "subdaemon": "openingd:" + badopeningd,
                "start": False,
    ...
    with open(
        os.path.join(l1.daemon.lightning_dir, TEST_NETWORK, "openingd-real"), "w"
    ) as f:
...
def test_getlog(node_factory):
    ...
    assert [
        ...
        for l in logs
        if l["type"]
        not in ("BROKEN", "UNUSUAL", "INFO", "DEBUG", "TRACE", "IO_IN", "IO_OUT")
    ] == []
...
def test_log_filter(node_factory):
    ...
    l1 = node_factory.get_node(
        options={"log-level": ["debug", "broken:022d223620"]}, old_hsmsecret=True
    )
    ...
    l2.daemon.early_opts += [
        f"--log-file={l}" for l in [log2] + l2.daemon.opts["log-file"]
    ]
    del l2.daemon.opts["log-file"]
    l2.daemon.opts["log-level"] = [
        "broken",  # broken messages go everywhere
...
def test_log_filter_bug(node_factory):
    ...
    l1 = node_factory.get_node(
        options={"plugin": log_plugin, "log-level": ["info", "debug:plugin-log"]}
    )
...
def test_force_feerates(node_factory):
    ...
    estimates = [
        {"blockcount": 2, "feerate": 15000, "smoothed_feerate": 15000},
        {"blockcount": 6, "feerate": 11000, "smoothed_feerate": 11000},
        {"blockcount": 12, "feerate": 7500, "smoothed_feerate": 7500},
        {"blockcount": 100, "feerate": 3750, "smoothed_feerate": 3750},
    ]
    ...
    assert (
        l1.rpc.listconfigs()["configs"]["force-feerates"]["value_str"]
...
def test_datastoreusage(node_factory):
    ...
    l1: LightningNode = node_factory.get_node(
        options={"disable-plugin": ["cln-xpay", "bookkeeper"]}
    )
    assert l1.rpc.datastoreusage() == {
        "datastoreusage": {"key": "[]", "total_bytes": 0}
    }

    data = "somedatatostoreinthedatastore"  # len 29
    l1.rpc.datastore(key=["a", "b"], string=data)
    assert l1.rpc.datastoreusage() == {
        "datastoreusage": {"key": "[]", "total_bytes": (29 + 1 + 1 + 1)}
    }
    assert l1.rpc.datastoreusage(key="a") == {
        "datastoreusage": {"key": "[a]", "total_bytes": (29 + 1 + 1 + 1)}
    }
    assert l1.rpc.datastoreusage(key=["a", "b"]) == {
        "datastoreusage": {"key": "[a,b]", "total_bytes": (29 + 1 + 1 + 1)}
    }
    ...
    assert l1.rpc.datastoreusage() == {
        "datastoreusage": {
            "key": "[]",
            "total_bytes": (29 + 1 + 1 + 1 + 29 + 1 + 1 + 1),
        }
    }
    assert l1.rpc.datastoreusage(key=["a", "b"]) == {
        "datastoreusage": {"key": "[a,b]", "total_bytes": (29 + 1 + 1 + 1)}
    }
    assert l1.rpc.datastoreusage(key=["a", "c"]) == {
        "datastoreusage": {"key": "[a,c]", "total_bytes": (29 + 1 + 1 + 1)}
    }

    # check that the key is also counted as stored data
    l1.rpc.datastore(
        key=["a", "thisissomelongkeythattriestostore46bytesofdata"], string=data
    )
    assert l1.rpc.datastoreusage() == {
        "datastoreusage": {"key": "[]", "total_bytes": (29 + 1 + 1 + 46 + 64)}
    }
    assert l1.rpc.datastoreusage(
        ...
        "datastoreusage": {
            "key": "[a,thisissomelongkeythattriestostore46bytesofdata]",
            "total_bytes": (29 + 1 + 1 + 46),
        }
    }

    # check that the root is also counted
    l1.rpc.datastore(
        key=["thisissomelongkeythattriestostore46bytesofdata", "a"], string=data
    )
    assert l1.rpc.datastoreusage(
        key=["thisissomelongkeythattriestostore46bytesofdata", "a"]
    ) == {
        "datastoreusage": {
            "key": "[thisissomelongkeythattriestostore46bytesofdata,a]",
            "total_bytes": (29 + 1 + 1 + 46),
    ...
    assert l1.rpc.datastoreusage(key=["a", "d", "e", "f", "g"]) == {
        "datastoreusage": {
            "key": "[a,d,e,f,g]",
            "total_bytes": (29 + 1 + 1 + 1 + 1 + 1 + 4),
        }
    }
    assert l1.rpc.datastoreusage() == {
        "datastoreusage": {
            "key": "[]",
            "total_bytes": (29 + 1 + 1 + 1 + 1 + 1 + 4 + 218),
        }
    }


@unittest.skipIf(
    os.getenv("TEST_DB_PROVIDER", "sqlite3") != "sqlite3", "This test requires sqlite3"
)
...
def test_field_filter(node_factory, chainparams):
    ...
    addr2 = (
        "2MxqzNANJNAdMjHQq8ZLkwzooxAFiRzXvEz"
        if not chainparams["elements"]
        else "XGx1E2JSTLZLmqYMAo3CGpsco85aS7so33"
    )
    ...
    dec = l1.rpc.call(
        "decode",
        {"string": inv["bolt11"]},
        filter={"currency": True, "payment_hash": True},
    )
    assert dec == {
    ...
    dec = l1.rpc.call(
        "decode",
        {"string": inv["bolt11"]},
        filter={"currency": True, "payment_hash": True, "fallbacks": [{"type": True}]},
    )
    assert dec == {
        "currency": chainparams["bip173_prefix"],
        "payment_hash": inv["payment_hash"],
    ...
    # Nonexistent fields.
    dec = l1.rpc.call("decode", {"string": inv["bolt11"]}, filter={"foobar": True})
    assert dec == {}
    ...
    res = l1.rpc.call("decode", {"string": inv["bolt11"]}, filter={"currency": True})
...
def test_feerate_arg(node_factory):
    ...
    by_blocks = {2: 15000, 6: 11000, 12: 7500, 100: 3750}

    # Literal values:
    fees = {"9999perkw": 9999, "10000perkb": 10000 // 4, 10000: 10000 // 4}
    ...
    for block in range(12, 100):
        ...
        fee = by_blocks[12] + (block - 12) * (by_blocks[100] - by_blocks[12]) // (
            100 - 12
        )
...
def test_config_whitespace(node_factory):
    ...
    configfile = os.path.join(
        l1.daemon.opts.get("lightning-dir"), TEST_NETWORK, "config"
    )
...
def test_setconfig(node_factory, bitcoind):
    ...
    configfile = os.path.join(
        l2.daemon.opts.get("lightning-dir"), TEST_NETWORK, "config"
    )
    setconfigfile = configfile + ".setconfig"

    assert l2.rpc.listconfigs("min-capacity-sat")["configs"] == {
        "min-capacity-sat": {"source": "default", "value_int": 10000, "dynamic": True}
    }
    ...
    assert l2.rpc.check(
        command_to_check="setconfig", config="min-capacity-sat", val=500000
    ) == {"command_to_check": "setconfig"}
    ...
    with open(configfile, "r") as f:
        ...
        assert lines == [
            "# setconfig commented out (see config.setconfig): min-capacity-sat=500000",
            lines[1],
...
def test_setconfig_access(node_factory, bitcoind):
    ...
    if os.getenv("TEST_DB_PROVIDER", "sqlite3") == "sqlite3":
        options["wallet"] = "sqlite3://" + os.path.join(
            node_factory.directory, "l1.sqlite3"
        )

    l1 = node_factory.get_node(options=options)

    netconfigfile = os.path.join(
        l1.daemon.opts.get("lightning-dir"), TEST_NETWORK, "config"
    )
    ...
    assert l1.rpc.setconfig(config="min-capacity-sat", val=1000000) == {
        "config": {
            "config": "min-capacity-sat",
...
def test_setconfig_changed(node_factory, bitcoind):
    ...
    assert (
        l1.rpc.listconfigs(config="min-capacity-sat")["configs"]["min-capacity-sat"][
            "value_int"
        ]
        == 100
    )
    ...
    with pytest.raises(
        RpcError,
        match=f"Configfile {netconfigfile} line 1 changed from min-capacity-sat=100 to #some comment!",
    ):
        l1.rpc.check("setconfig", config="min-capacity-sat", val=9999)
    with pytest.raises(
        RpcError,
    ...
    with pytest.raises(
        RpcError, match=f"Configfile {setconfigfile} no longer has 2 lines"
    ):
        l1.rpc.check("setconfig", config="min-capacity-sat", val=9999)
    with pytest.raises(
        RpcError, match=f"Configfile {setconfigfile} no longer has 2 lines"
    ...
    with pytest.raises(
        RpcError,
        match=f"Configfile {setconfigfile} line 2 changed from min-capacity-sat=9999 to min-capacity-sat=999!",
    ):
        l1.rpc.check("setconfig", config="min-capacity-sat", val=9999)
    with pytest.raises(
        RpcError,
...
def test_recover_command(node_factory, bitcoind, old_hsmsecret):
    ...
    def get_hsm_secret(n):
        ...
        recover = (
            subprocess.check_output(
                ["tools/lightning-hsmtool", "getsecret", hsmfile, "leet"]
            )
            .decode("utf-8")
            .strip()
        )
    ...
    with open(
        os.path.join(l1.daemon.lightning_dir, f"lightningd-{TEST_NETWORK}.pid"), "r"
    ) as f:
        pid = f.read().strip()

    assert l1.rpc.check("recover", hsmsecret=l2recover) == {
    ...
    shutil.rmtree(
        os.path.join(
            l1.daemon.lightning_dir, TEST_NETWORK, f"lightning.pre-recover.{pid}"
        )
    )
    ...
    if old_hsmsecret:
        assert l1.rpc.check("recover", hsmsecret=l1hex) == {
            "command_to_check": "recover"
        }
        l1.rpc.recover(hsmsecret=l1hex)
    else:
        # Modern style requires mnemonic arg.
        assert l1.rpc.check("recover", hsmsecret=l1recover) == {
            "command_to_check": "recover"
        }
        l1.rpc.recover(hsmsecret=l1recover)
...
def test_even_sendcustommsg(node_factory):
    l1, l2 = node_factory.get_nodes(2, opts={"log-level": "io", "allow_warning": True})
    l1.connect(l2)
...
def test_set_feerate_offset(node_factory, bitcoind):
    ...
    assert l1.daemon.is_in_log("Server started with public key")
    configs = l1.rpc.listconfigs()["configs"]
    assert configs["commit-feerate-offset"] == {"source": "cmdline", "value_int": 100}
    ...
    if (
        "anchors/even"
        in only_one(l1.rpc.listpeerchannels()["channels"])["channel_type"]["names"]
    ):
        feerate = 3850
        min_feerate = 253
    else:
        ...
        min_feerate = 1875
    l1.daemon.wait_for_log(
        f"lightningd: update_feerates: feerate = {feerate}, "
...
def test_low_fd_limit(node_factory, bitcoind):
    ...
    l1, l2 = node_factory.line_graph(
        2,
        opts=[
            {"dev-fd-limit-multiplier": limits[1] + 1, "allow_warning": True},
            {"dev-fd-limit-multiplier": limits[1], "allow_warning": True},
        ],
    )
    ...
    assert l1.daemon.is_in_log(
        "UNUSUAL.*WARNING: we have 1 channels but file descriptors limited"
    )
...
def test_badparam_discretion(node_factory):
    ...
    with pytest.raises(
        RpcError, match="rune: should be base64 string: invalid token"
    ) as err:
        l1.rpc.checkrune(rune="THIS IS NOT ACTUALLY A RUNE")

    assert (
        err.value.error["message"]
        == "rune: should be base64 string: invalid token '\"THIS IS NOT ACTUALLY A RUNE\"'"
    )
    ...
    with pytest.raises(
        RpcError,
        match=r"rune: should be base64 string: invalid token \(see logs for details\)",
    ):
        l1.rpc.checkrune(rune="THIS IS NOT ACTUALLY A RUNE")

    l1.daemon.wait_for_log(
        r"checkrune: Invalid parameter rune \(should be base64 string\): token '\"THIS IS NOT ACTUALLY A RUNE\"'"
    )
...
def test_listaddresses(node_factory):
    ...
    with pytest.raises(
        RpcError,
        match="Could not parse destination address, address should be a valid address",
    ) as err:
    ...
    assert (
        err.value.error["message"]
        == "Could not parse destination address, address should be a valid address"
    )
    ...
    # Not our Address
    addresses = l1.rpc.listaddresses(
        address="bcrt1q3p9jh7x0mnx8tmx5meapksvcxytlxgwz907wc0"
    )["addresses"]
    assert len(addresses) == 0
    ...
    assert (
        addresses[0]["p2tr"]
        == "bcrt1ph9gd3vrxqv5c43lhz330n6u497utuqzzjwtrwj89wy879z6nwrpseaf4et"
    )
...
def test_tracing(node_factory):
    ...
    # Test parent trace
    trace_fnamebase = os.path.join(
        l1.daemon.lightning_dir, TEST_NETWORK, "l1.parent.trace"
    )
    l1.daemon.env["CLN_DEV_TRACE_FILE"] = trace_fnamebase
    l1.daemon.env["CLN_TRACEPARENT"] = (
...
def test_zero_locktime_blocks(node_factory, bitcoind):
    ...
    l1, l2, l3 = node_factory.line_graph(
        3, opts=[{}, {"watchtime-blocks": 0}, {}], wait_for_announce=True
    )

    # We should be able to use the channel and close it.
    inv = l3.rpc.invoice(
        10000, "test_zero_locktime_blocks", "test_zero_locktime_blocks"
    )
...
def test_filter_with_invalid_json(node_factory):
    ...
    l1.daemon.early_opts = []
    l1.daemon.opts = {
        k: v for k, v in l1.daemon.opts.items() if not k.startswith("dev")
    }
    l1.start()
