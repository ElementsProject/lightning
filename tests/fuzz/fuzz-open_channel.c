...
static u8 *test_sync_read(const tal_t *ctx, int fd)
{
	...
	{
		/* Choose between creating a valid message and a fuzzer one. */
...
static struct state *fromwire_new_state(const tal_t *ctx)
{
	...
	u8 *openingd_init_msg = tal_arr(ctx, u8, 0);

	/* Create the first few fields of the openingd_init_msg.
	 * This makes discovering interesting paths faster.
	 */
	struct feature_set *our_features = fromwire_feature_set(ctx, cursor, max);
	if (!our_features)
		return NULL;
	...
	if (*max)
		*cursor += remaining_len, *max -= remaining_len;

	if (!fromwire_openingd_init(ctx, openingd_init_msg,
					&chainparams,
					&state->our_features,
...
void run(const u8 *data, size_t size)
{
	...
	if (fromwire_u8(cursor, max) % 2)
		...
		u8 *fuzz_msg = create_fuzz_msg(tmpctx);
