...
void run(const u8 *data, size_t size)
{
    ...
    tcase.their_funds = fromwire_amount_sat(&data, &size);
    tcase.available_funds = fromwire_amount_sat(&data, &size);
    ...
    if (flag)
    {
        if (size < 8)
			return;
        our_last_funds_val = fromwire_amount_sat(&data, &size);
    ...
    tcase.channel_size = fromwire_amount_sat(&data, &size);
    tcase.lease_request = fromwire_amount_sat(&data, &size);

    if (size < 1)
		return;
    tcase.policy.opt = (enum funder_opt)(fromwire_u8(&data, &size) % 3);
    tcase.policy.mod = fromwire_u64(&data, &size);
    tcase.policy.min_their_funding = fromwire_amount_sat(&data, &size);
    tcase.policy.max_their_funding = fromwire_amount_sat(&data, &size);
    tcase.policy.per_channel_max = fromwire_amount_sat(&data, &size);
    tcase.policy.per_channel_min = fromwire_amount_sat(&data, &size);

    if (size < 1)
		return;
    tcase.policy.fuzz_factor = fromwire_u8(&data, &size) % 101;

    tcase.policy.reserve_tank = fromwire_amount_sat(&data, &size);
    ...
    /* Validate invariants */
    if (err != NULL) {
        /* On error, our_funds must be 0 */
        if (!amount_sat_eq(our_funds, AMOUNT_SAT(0))) {
            fprintf(stderr, "Error returned but our_funds != 0\n");
            abort();
        }
    } else {
