...
#define status_failed test_status_failed
...
void test_status_failed(enum status_failreason reason, const char *fmt, ...)
{
	longjmp(exit_jmp, 1);
}
...
static u8 *create_gossip_msg(const tal_t *ctx, const u8 **cursor, size_t *max, enum peer_wire t)
{
	...
	switch (t)
	{
		...
		case WIRE_CHANNEL_UPDATE:
		{
			...
			secp256k1_ecdsa_signature_serialize_compact(secp256k1_ctx,
								compact_sig,
								&sig);
			memcpy(cupdate, compact_sig, sizeof(compact_sig));
