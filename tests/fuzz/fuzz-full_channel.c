...
static bool update_feerate(struct channel *channel, u32 feerate)
{
	...
	exchange_commits(channel, channel->opener, NULL);
...
static void update_blockheight(struct channel *channel, u32 blockheight)
{
	...
	exchange_commits(channel, channel->opener, NULL);
	assert(get_blockheight(channel->blockheight_states, channel->opener, LOCAL) == blockheight);
	assert(get_blockheight(channel->blockheight_states, channel->opener, REMOTE) == blockheight);
}
		     ...
		     struct amount_msat msatoshi, u32 cltv)
{
	...
	if (channel_add_htlc(channel, sender, id, msatoshi, cltv, &rhash,
		dummy_routing, NULL, NULL, NULL, true) != CHANNEL_ERR_ADD_OK) {
		tal_free(dummy_routing);
		return false;
	}
	assert(channel_get_htlc(channel, sender, id));

	changed_htlcs = tal_arr(channel, const struct htlc *, 0);
	exchange_commits(channel, sender, &changed_htlcs);

	tal_free(dummy_routing);
	tal_free(changed_htlcs);
	return true;
}
			 ...
			 u64 id, const struct preimage *preimage)
{
	...
	changed_htlcs = tal_arr(channel, const struct htlc *, 0);
	exchange_commits(channel, !original_sender, &changed_htlcs);
	assert(!channel_get_htlc(channel, original_sender, id));
...
void run(const u8 *data, size_t size)
{
	...
	while (size > 0) {
		int op = fromwire_u8(&data, &size) % 5;
		switch (op) {
		case 0: /* ADD_HTLC */
		{
