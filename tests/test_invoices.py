...
def test_waitanyinvoice(node_factory, executor):
    ...
    # If immediate is True and a paid invoice is not yet
    # available, it should fail immediately.
    with pytest.raises(RpcError):
        l2.rpc.waitanyinvoice(pay_index, True)

    # If immediate is True but a paid invoice is available
    # anyway, it should return successfully immediately.
    l1.rpc.pay(inv4['bolt11'])
    r = executor.submit(l2.rpc.waitanyinvoice, pay_index, True).result(timeout=5)
    assert r['label'] == 'inv4'
