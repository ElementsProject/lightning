...
KEYS_TO_ADD_IN_REPLACE_RESPONSE = ['timestamp', 'created_at', 'paid_at', 'completed_at', 'expires_at', 'payment_preimage', 'payment_hash', 'signature', 'payment_id', 'outpoint', 'txid', 'origin', 'account', 'tx', 'psbt', 'bolt11', 'payload', 'assocdata', 'onion''invreq_id', 'invoice', 'bolt12']

REPLACE_RESPONSE_VALUES = {
  re.compile(re.escape(CWD)): '/root/lightning',
  re.compile(r'/tmp/ltests-[^/]+/test_generate_examples_[^/]+/lightning-[^/]+'): '/tmp/.lightning',
}

REPLACE_VALUE_RULES = [
  {'key': 'timestamp','init': 1722, 'append': '00', 'change': 1, 'multiple': 3},
  {'key': 'created_at','init': 1737, 'append': '00', 'change': 1, 'multiple': 3},
  {'key': 'paid_at','init': 173711, 'append': '00', 'change': 1, 'multiple': 2},
  {'key': 'completed_at','init': 173722, 'append': '00', 'change': 1, 'multiple': 2},
  {'key': 'expires_at','init': 1738, 'append': '00', 'change': 1, 'multiple': 3},
 
  {'key': 'payload', 'init': 'payload', 'append': '00', 'change': 1, 'multiple': 40},
  {'key': 'assocdata', 'init': 'assocdata0', 'append': '00', 'change': 1, 'multiple': 27},
  {'key': 'onion', 'init': 'onion0', 'append': '00', 'change': 1, 'multiple': 1363},
  {'key': 'invreq_id', 'init': 'invreqid', 'append': '00', 'change': 1, 'multiple': 28},
  {'key': 'invoice', 'init': 'lni1qqg0qe', 'append': '00', 'change': 1, 'multiple': 415},
  {'key': 'bolt12', 'init': 'lnr1qqgzvzr2g67cp0qsc6dr', 'append': '00', 'change': 1, 'multiple': 150},
  {'key': 'bolt11', 'init': 'lnbcrt100n1pnt2000000bolt110', 'append': '00', 'change': 1, 'multiple': 103},
  {'key': 'psbt', 'init': 'cHNidP8BAgQCAAAAAQMEbwAAAAEEAQpsbt', 'append': '00', 'change': 1, 'multiple': 43},
  {'key': 'tx', 'init': '02000000000101txtxtx', 'append': '00000', 'change': 1, 'multiple': 100},
  {'key': 'payment_preimage', 'init': 'paymentpreimage0', 'append': '00', 'change': 1, 'multiple': 24},
  {'key': 'payment_hash', 'init': 'paymenthash0', 'append': '00', 'change': 1, 'multiple': 27},
  {'key': 'signature', 'init': 'signature0', 'append': '00', 'change': 1, 'multiple': 66},
  {'key': 'payment_id', 'init': 'paymentid0', 'append': '00', 'change': 1, 'multiple': 27},
  {'key': 'outpoint', 'init': 'outpoint', 'append': '00', 'change': 1, 'multiple': 28, 'last': ':0'},
  {'key': 'txid', 'init': 'txid', 'append': '00', 'change': 1, 'multiple': 30},
  {'key': 'origin', 'init': 'origin', 'append': '00', 'change': 1, 'multiple': 29},
  {'key': 'account', 'init': 'account0', 'append': '00', 'change': 1, 'multiple': 28, 'except': ['wallet']},

  {'key': 'credit_msat', 'init': 51, 'append': '00', 'change': 1, 'multiple': 2, 'except': [0], 'last': '000'},
  {'key': 'debit_msat', 'init': 48, 'append': '00', 'change': 1, 'multiple': 2, 'except': [0], 'last': '000'},
]
...
def update_example(node, method, params, res=None, description=None, execute=True, filename=None):
  ...
  try:
    def replace_values_in_json(data, data_key):
      if isinstance(data, dict):
        ...
        for replace_rule in REPLACE_VALUE_RULES:
          if data_key == replace_rule['key'] and all(data != exception for exception in replace_rule.get('except', [])) and isinstance(replace_rule['init'], str) and not data.startswith(replace_rule['init']):
            ...
            if data_key in KEYS_TO_ADD_IN_REPLACE_RESPONSE:
              REPLACE_RESPONSE_VALUES.update({data: new_value})
            break
        for pattern, replacement in REPLACE_RESPONSE_VALUES.items():
        ...
        for replace_rule in REPLACE_VALUE_RULES:
          if data_key == replace_rule['key'] and not str(data).startswith(str(replace_rule['init'])):
            new_value = int(f"{replace_rule['init']}{replace_rule['append'] * replace_rule['multiple']}{replace_rule.get('last', '')}")
            replace_rule['append'] = f"{(int(replace_rule['append']) + replace_rule['change']):02}"
            if data_key in KEYS_TO_ADD_IN_REPLACE_RESPONSE:
              REPLACE_RESPONSE_VALUES.update({data: new_value})
            break
        return REPLACE_RESPONSE_VALUES.get(data, data)
      else:
        return data
    ...
    with open(file_path, 'r+', encoding='utf-8') as file:
      ...
      original_res = node.rpc.call(method, params) if execute else None
...
def setup_test_nodes(node_factory, bitcoind):
  ...
  try:
    ...
    REPLACE_RESPONSE_VALUES.update({
      l1.info['version']: 'v24.08',
      l1.info['alias']: 'JUNIORBEAM',
      l1.info['binding'][0]['port']: 19734,
      f'127.0.0.1:{l1.info["binding"][0]["port"]}': '127.0.0.1:19734',
      l2.info['alias']: 'SILENTARTIST',
      l2.info['binding'][0]['port']: 19735,
      f'127.0.0.1:{l2.info["binding"][0]["port"]}': '127.0.0.1:19735',
      l3.info['alias']: 'HOPPINGFIRE',
      l3.info['binding'][0]['port']: 19736,
      f'127.0.0.1:{l3.info["binding"][0]["port"]}': '127.0.0.1:19736',
      l4.info['alias']: 'JUNIORFELONY',
      l4.info['binding'][0]['port']: 19737,
      f'127.0.0.1:{l4.info["binding"][0]["port"]}': '127.0.0.1:19737',
      l5.info['alias']: 'SOMBERFIRE',
      l5.info['binding'][0]['port']: 19738,
      f'127.0.0.1:{l5.info["binding"][0]["port"]}': '127.0.0.1:19738',
      l6.info['alias']: 'LOUDPHOTO',
      l6.info['binding'][0]['port']: 19739,
      f'127.0.0.1:{l6.info["binding"][0]["port"]}': '127.0.0.1:19739',
    })
    # Upgrade wallet
    ...
    REPLACE_RESPONSE_VALUES.update({
      c12: '109x1x0',
      c12res['tx']: 'tx02000000000101txtxtx' + ('12000' * 100),
      c12res['txid']: 'txidchannel012000012000012000012000012000012000012000012000012000',
      c12res['channel_id']: 'channelid0120000120000120000120000120000120000120000120000120000',
      c23: '111x1x1',
      c23res['tx']: 'tx02000000000101txtxtx' + ('23000' * 100),
      c23res['txid']: 'txidchannel023000023000023000023000023000023000023000023000023000',
      c23res['channel_id']: 'channelid0230000230000230000230000230000230000230000230000230000',
      c34: '113x1x0',
      c34res['tx']: 'tx02000000000101txtxtx' + ('34000' * 100),
      c34res['txid']: 'txidchannel034000034000034000034000034000034000034000034000034000',
      c34res['channel_id']: 'channelid0340000340000340000340000340000340000340000340000340000',
      c25: '115x1x1',
      c25res['tx']: 'tx02000000000101txtxtx' + ('25000' * 100),
...
def generate_transactions_examples(l1, l2, l3, l4, l5, c25, bitcoind):
  ...
  try:
    ...
    REPLACE_RESPONSE_VALUES.update({
        inv_l31['payment_hash']: 'paymenthashinvl0310003100031000310003100031000310003100031000310',
        ...
        inv_l31['expires_at']: 1731100000,
        inv_l32['payment_hash']: 'paymenthashinvl0320003200032000320003200032000320003200032000320',
        inv_l32['bolt11']: 'lnbcrt100n1pnt2000000bolt11invl032000000000bolt11invl032000000000bolt11invl032000000000bolt11invl032000000000bolt11invl032000000000bolt11invl032000000000bolt11invl032000000000bolt11invl032000000000bolt11invl032000000000bolt11invl032000',
        inv_l32['payment_secret']: 'paymentsecretinvl00320003200032000320003200032000320003200032000',
        inv_l32['expires_at']: 1732100000,
    })
    ...
    REPLACE_RESPONSE_VALUES.update({
        inv_l11['payment_hash']: 'paymenthashinvl0110001100011000110001100011000110001100011000310',
        ...
        inv_l11['payment_secret']: 'paymentsecretinvl00110001100011000110001100011000110001100011000',
        inv_l11['expires_at']: 1711100000,
        ...
        inv_l21['expires_at']: 1722100000,
        inv_l22['payment_hash']: 'paymenthashinvl0220002200022000220002200022000220002200022000310',
        ...
        inv_l22['payment_secret']: 'paymentsecretinvl00220002200022000220002200022000220002200022000',
        inv_l22['expires_at']: 1722100000,
        inv_l33['payment_hash']: 'paymenthashinvl0330003300033000330003300033000330003300033000310',
        inv_l33['bolt11']: 'lnbcrt100n1pnt2000000bolt11invl033000000000bolt11invl033000000000bolt11invl033000000000bolt11invl033000000000bolt11invl033000000000bolt11invl033000000000bolt11invl033000000000bolt11invl033000000000bolt11invl033000000000bolt11invl033000',
        inv_l33['payment_secret']: 'paymentsecretinvl00330003300033000330003300033000330003300033000',
        inv_l33['expires_at']: 1733100000,
        inv_l34['payment_hash']: 'paymenthashinvl0340003400034000340003400034000340003400034000310',
        inv_l34['bolt11']: 'lnbcrt100n1pnt2000000bolt11invl034000000000bolt11invl034000000000bolt11invl034000000000bolt11invl034000000000bolt11invl034000000000bolt11invl034000000000bolt11invl034000000000bolt11invl034000000000bolt11invl034000000000bolt11invl034000',
        inv_l34['payment_secret']: 'paymentsecretinvl00340003400034000340003400034000340003400034000',
        inv_l34['expires_at']: 1734100000,
    })
    ...
    REPLACE_RESPONSE_VALUES.update({
      c23: '123x1x1',
      c23res['tx']: 'tx02000000000101txtxtx' + ('23000' * 100),
      c23res['txid']: 'txidchannel02300002300002300002300002300002300002300002300002300',
      c23res['channel_id']: 'channelid0230000230000230000230000230000230000230000230000230000',
      c34: '125x1x0',
      c34res['tx']: 'tx02000000000101txtxtx' + ('34000' * 100),
      c34res['txid']: 'txidchannel03400003400003400003400003400003400003400003400003400',
      c34res['channel_id']: 'channelid0340000340000340000340000340000340000340000340000340000',
    })
    ...
    REPLACE_RESPONSE_VALUES.update({
        inv_l12['payment_hash']: 'paymenthashinvl0120001200012000120001200012000120001200012000310',
        inv_l12['bolt11']: 'lnbcrt100n1pnt2000000bolt11invl012000000000bolt11invl012000000000bolt11invl012000000000bolt11invl012000000000bolt11invl012000000000bolt11invl012000000000bolt11invl012000000000bolt11invl012000000000bolt11invl012000000000bolt11invl012000',
        inv_l12['payment_secret']: 'paymentsecretinvl00120001200012000120001200012000120001200012000',
        inv_l12['expires_at']: 1712100000,
        inv_l24['payment_hash']: 'paymenthashinvl0240002400024000240002400024000240002400024000310',
        inv_l24['bolt11']: 'lnbcrt100n1pnt2000000bolt11invl024000000000bolt11invl024000000000bolt11invl024000000000bolt11invl024000000000bolt11invl024000000000bolt11invl024000000000bolt11invl024000000000bolt11invl024000000000bolt11invl024000000000bolt11invl024000',
        inv_l24['payment_secret']: 'paymentsecretinvl00240002400024000240002400024000240002400024000',
        inv_l24['expires_at']: 1724100000,
        inv_l25['payment_hash']: 'paymenthashinvl0250002500025000250002500025000250002500025000310',
        inv_l25['bolt11']: 'lnbcrt100n1pnt2000000bolt11invl025000000000bolt11invl025000000000bolt11invl025000000000bolt11invl025000000000bolt11invl025000000000bolt11invl025000000000bolt11invl025000000000bolt11invl025000000000bolt11invl025000000000bolt11invl025000',
        inv_l25['payment_secret']: 'paymentsecretinvl00250002500025000250002500025000250002500025000',
        inv_l25['expires_at']: 1725100000,
        inv_l26['payment_hash']: 'paymenthashinvl0260002600026000260002600026000260002600026000310',
    ...
    REPLACE_RESPONSE_VALUES.update({
        inv_l13['payment_hash']: 'paymenthashinvl0130001300013000130001300013000130001300013000310',
        inv_l13['bolt11']: 'lnbcrt100n1pnt2000000bolt11invl013000000000bolt11invl013000000000bolt11invl013000000000bolt11invl013000000000bolt11invl013000000000bolt11invl013000000000bolt11invl013000000000bolt11invl013000000000bolt11invl013000000000bolt11invl013000',
        inv_l13['payment_secret']: 'paymentsecretinvl00130001300013000130001300013000130001300013000',
        inv_l13['expires_at']: 1713100000,
    })
...
def generate_offers_renepay_examples(l1, l2, inv_l21, inv_l34):
  ...
  try:
    ...
    REPLACE_RESPONSE_VALUES.update({
      offer_l21['offer_id']: 'offeridl21000002100000210000021000002100000210000021000002100000',
      ...
      offer_l22['offer_id']: 'offeridl22000002200000220000022000002200000220000022000002200000',
      offer_l22['bolt12']: 'lno1qgsq000000bolt12000220002200022000220002200022000220002200022000220002200022000220002200022000220002200022000220002200022000220002200022',
...
def generate_splice_examples(node_factory, bitcoind):
  ...
  try:
    ...
    REPLACE_RESPONSE_VALUES.update({
      l7.info['alias']: 'SILENTTRINITY',
      l7.info['binding'][0]['port']: 19740,
      f'127.0.0.1:{l7.info["binding"][0]["port"]}': '127.0.0.1:19740',
      l8.info['alias']: 'GREENPLOW',
      l8.info['binding'][0]['port']: 19741,
      f'127.0.0.1:{l8.info["binding"][0]["port"]}': '127.0.0.1:19741',
      c78: '142x1x0',
      c78res['tx']: 'tx02000000000101txtxtx' + ('78000' * 100),
      c78res['txid']: 'txidchannel07800007800007800007800007800007800007800007800007800',
      c78res['channel_id']: 'channelid0780000780000780000780000780000780000780000780000780000',
    })
    # Splice
...
def generate_channels_examples(node_factory, bitcoind, l1, l3, l4, l5):
  ...
  try:
    ...
    REPLACE_RESPONSE_VALUES.update({
      l9.info['alias']: 'SOMBERCHIPMUNK',
      l9.info['binding'][0]['port']: 19742,
      f'127.0.0.1:{l9.info["binding"][0]["port"]}': '127.0.0.1:19742',
      l10.info['alias']: 'LIGHTNINGWALK',
      l10.info['binding'][0]['port']: 19743,
      f'127.0.0.1:{l10.info["binding"][0]["port"]}': '127.0.0.1:19743',
    })
    amount = 2 ** 24
    ...
    REPLACE_RESPONSE_VALUES.update({
      l11.info['alias']: 'SLICKERMONTANA',
      l11.info['binding'][0]['port']: 19744,
      f'127.0.0.1:{l11.info["binding"][0]["port"]}': '127.0.0.1:19744',
      l12.info['alias']: 'STRANGEWAFFLE',
      l12.info['binding'][0]['port']: 19745,
      f'127.0.0.1:{l12.info["binding"][0]["port"]}': '127.0.0.1:19745',
      c1112res['tx']: 'tx02000000000101txtxtx' + ('01112' * 100),
      c1112res['txid']: 'txidchannel01112001112001112001112001112001112001112001112001112',
      c1112res['channel_id']: 'channelid0111200111200111200111200111200111200111200111200111200',
    })
    logger.info(f'Replace List: {REPLACE_RESPONSE_VALUES}')
...
def generate_autoclean_delete_examples(l1, l2, l3, l4, l5, c12, c23):
  ...
  try:
    ...
    REPLACE_RESPONSE_VALUES.update({
      c24: '159x1x0',
      c24res['tx']: 'tx02000000000101txtxtx' + ('24000' * 100),
      c24res['txid']: 'txidchannel02400002400002400002400002400002400002400002400002400',
      c24res['channel_id']: 'channelid0240000240000240000240000240000240000240000240000240000',
    })
