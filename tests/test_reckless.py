...
def canned_github_server(directory):
    ...
    copy_plugins = f"cp -r {plugins_path}/* .;"

    subprocess.check_output([copy_plugins], env=my_env, shell=True, cwd=repo_dir)

    # Create requirements.txt file for the testpluginpass
    # with pyln-client installed from the local source
    requirements_file_path = os.path.join(repo_dir, "testplugpass", "requirements.txt")
...
def test_sources(node_factory):
    ...
    print(r)
    assert "https://github.com/lightningd/plugins" in r["sources"]
...
def test_local_dir_install(node_factory):
    ...
    assert source_dir not in r["sources"]
...
def test_tag_install(node_factory):
    ...
    assert r["uninstall"]["plugin_name"] == "testplugpass"
    r = node.rpc.call("reckless", ["install", "-v", "testplugpass@v1"])
    assert r["install"]["enabled"]
    assert r["install"]["plugin_name"] == "testplugpass"
    assert r["install"]["installed_commit"] == "v1"
...
def test_reckless_uv_install(node_factory):
    ...
    assert r["install"]["enabled"]
    assert r["install"]["plugin_name"] == "testpluguv"
...
def test_reckless_manifest(node_factory):
    ...
    assert r["install"]["enabled"]
    assert r["install"]["plugin_name"] == "testplugmani"
    assert node.rpc.testmethod() == "I live."

    manifest = node.lightning_dir / "reckless/testplugmani/.metadata.json"
    with open(manifest, "r") as f:
        data = json.load(f)

    data["manifest"]["offer"] = bolt12
...
@unittest.skipIf(VALGRIND, "virtual environment triggers memleak detection")
def test_update_with_options_is_noop(node_factory):
    """Updating a plugin that has configured options to the same
    version should be a noop."""
    ...
    config = node.rpc.listconfigs("required-opt")["configs"]
    assert config["required-opt"]["value_str"] == "foo"

    # No newer version exists, but `update` still goes through the
    # disable -> enable cycle.
    r = node.rpc.call("reckless", ["update", "-v", "testplugreqopts"])
    assert r["update"][0]["plugin_name"] == "testplugreqopts"
    assert r["update"][0]["enabled"], (
        "a no-op update must not disable a plugin that has configured options"
    )
