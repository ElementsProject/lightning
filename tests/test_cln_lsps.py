...
from utils import only_one, sync_blockheight
...
def test_lsps2_mpp_two_upstream_chans(node_factory, bitcoind):
    ...
    # join_nodes([l4, l2]) above only syncs l4 and l2 to the tip; l1 and l3
    # can still be catching up to the blocks it mined. sendpay builds its
    # route from the payer's own blockheight, so a stale l3 would pick a
    # lower cltv and its part can arrive at l1 with too little headroom.
    # Bring everyone to the tip before paying.
...
def test_lsps2_restart_settlement_channel_force_close_abandoned(node_factory, bitcoind):
    ...
    """
    hold_plugin = os.path.join(os.path.dirname(__file__), "plugins/hold_htlcs.py")
    # allow_bad_gossip: the LSP kills the JIT channel on the HTLC deadline
    # while l1 still thinks it is alive, so l1's post-reconnect channel_update
    # for the dead channel's alias can race in as "Bad gossip order".
    l1, l2, l3, chanid = setup_lsps2_network(
        node_factory,
        bitcoind,
        lsp_opts={**LSP_OPTS, "allow_bad_gossip": True},
        client_opts={"plugin": hold_plugin, "hold-time": 10000},
        may_reconnect=True,
    )
    # JIT channels can trigger bookkeeper "Unable to calculate fees" on restart.
...
def test_lsps2_channel_ready_wait_is_bounded(node_factory, bitcoind):
    ...
    # state the CVE gets stuck in. Wait for CHANNELD_AWAITING_LOCKIN
    # specifically: the transient OPENINGD state appears at fundchannel_start,
    # before the funding inputs are reserved, and would race the assert below.
    wait_for(
        lambda: [
            c["state"] for c in l2.rpc.listpeerchannels(l1.info["id"])["channels"]
        ]
