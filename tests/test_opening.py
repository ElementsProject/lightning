...
def test_zeroconf_open(bitcoind, node_factory):
    ...
    # An invoice generated by l2 must still use the remote_alias
    inv = l1.rpc.invoice(10**5, 'lbl1', 'desc')['bolt11']
    details = l1.rpc.decodepay(inv)
    # A single routehint, with a single hop, and it should be using the remote_alias.
    assert len(details['routes']) == 1
    assert len(details['routes'][0]) == 1
    assert details['routes'][0][0]['short_channel_id'] == incoming[0]["remote_alias"]
