...
from utils import EXPERIMENTAL_FEATURES, only_one, wait_for, sync_blockheight
...
@unittest.skipIf(not EXPERIMENTAL_FEATURES, "dual funding is experimental")
def test_burn_on_cancel(node_factory, bitcoind):
    # If our peer cancels us, we should burn the tx
    plugin_path = os.path.join(os.getcwd(), 'tests/plugins/funder.py')
    ...
    l2.daemon.wait_for_log(r'1 utxo burned in tx')
    only_one(l2.rpc.listfunds()['outputs'])
    assert len(l2.rpc.listpeers()['peers']) == 0

    l1.rpc.connect(l2.info['id'], 'localhost', l2.port)
    assert len(only_one(l1.rpc.listpeers()['peers'])['channels']) == 0
    assert len(only_one(l2.rpc.listpeers()['peers'])['channels']) == 0
