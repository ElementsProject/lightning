...
def test_gossip_store_compact(node_factory, bitcoind):
    ...
    # Now compact store.
    l2.rpc.call('dev-compact-gossip-store')
    l2.daemon.wait_for_logs(['gossipd: compaction done: 2362 -> 2062 bytes',
                             'connectd: Reopened gossip_store, reduced to offset 2062'])
...
def test_gossip_store_compact_while_extending(node_factory, bitcoind, executor):
    ...
    l1.daemon.wait_for_logs(['gossipd: compaction done',
                             'connectd: Reopened gossip_store, reduced to offset 2245'])

    post_channels = l1.rpc.listchannels()
    post_nodes = l1.rpc.listnodes()
    l1.daemon.wait_for_log('topology: Reopened gossip_store, reduced to offset 2245')
...
def test_gossip_store_compact_miss_update(node_factory, bitcoind, executor):
    """If we compact twice, you have to reload"""
    l2 = setup_gossip_store_test(node_factory, bitcoind)
    ...
    l2.daemon.wait_for_log('topology: Reopened gossip_store, but we missed some.*, so reloading')
