...
def test_bkpr_report_tags_and_fallback(node_factory):
    ...
    res = l1.rpc.call(
        "bkpr-report",
        {
            "format": "{tag}|{acct_name}|{outpoint?NONE}|{txid?NONE}|{payment_id?NONE}|{bkpr-currency?NONE}|{currencyrate?NONE}",
            "headers": ['tag",acct_name,outpoint,txid,payment_id,bkpr-currency,currencyrate', ",,,,,,"],
        },
    )

    assert "report" in res
    # Header lines copied literally (including ")
    assert res["report"][0] == 'tag",acct_name,outpoint,txid,payment_id,bkpr-currency,currencyrate'
    assert res["report"][1] == ",,,,,,"
    ...
    res = l1.rpc.bkpr_report(format="{tag}|{acct_name}|{credit}|{debit}|{creditdebit}|{currencycredit}|{currencydebit}|{currencycreditdebit}")
...
def test_bkpr_report_lightning_cli_csv(node_factory):
    l1, l2 = node_factory.line_graph(2, opts={'log-level': 'io'})
