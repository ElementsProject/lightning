...
@pytest.mark.slow_test
@pytest.mark.developer("requres 'dev-queryrates'")
def test_channel_lease_closes(node_factory, bitcoind):
    '''
    ...
    l4-l2: l4 leases funds from l2; mutual close at the end of the lease
    '''

    l1, l2, l3, l4 = node_factory.get_nodes(4, opts={'funder-policy': 'match',
    ...
    # free up some funds for l2 to fund a channel with l4
    bitcoind.generate_block(1, wait_for_mempool=1)
    sync_blockheight(bitcoind, [l2])

    # l4 leases a channel from l2
    l4.rpc.connect(l2.info['id'], 'localhost', l2.port)
    # l4 leases a channel to l2
    l4.rpc.fundchannel(l2.info['id'], amount, request_amt=amount,
                       feerate='{}perkw'.format(feerate),
                       compact_lease=rates['compact_lease'])
    ...
    fundings = only_one(only_one(l1.rpc.listpeers()['peers'])['channels'])['funding_msat']
    assert Millisatoshi(est_fees + amount * 1000) == Millisatoshi(fundings[l2.info['id']])

    bitcoind.generate_block(6)
    l1.daemon.wait_for_log('to CHANNELD_NORMAL')
    l3.daemon.wait_for_log('to CHANNELD_NORMAL')
    l4.daemon.wait_for_log('to CHANNELD_NORMAL')
    ...
    wait_for(lambda: [c['active'] for c in l4.rpc.listchannels(l4.get_channel_scid(l2))['channels']] == [True, True])
    ...
    inv = l2.rpc.invoice(10**4, '4', 'no_4')
    ...
    with pytest.raises(RpcError, match=r'Peer leased this channel from us'):
        l2.rpc.close(l4.get_channel_scid(l2))

    bitcoind.generate_block(6)
    sync_blockheight(bitcoind, [l1, l2, l3, l4])
    # make sure we're at the right place for the csv lock
    l2.daemon.wait_for_log('Blockheight: SENT_ADD_ACK_COMMIT->RCVD_ADD_ACK_REVOCATION LOCAL now 112')
    l2.stop()
    ...
    # We need to give l4-l2 time to update their blockheights
    bitcoind.generate_block(1000)
    sync_blockheight(bitcoind, [l2, l3, l4])
    l4.daemon.wait_for_log('peer_out WIRE_UPDATE_BLOCKHEIGHT')
    ...
    # l2<->l4 mutual close should work
    chan = l4.get_channel_scid(l2)
