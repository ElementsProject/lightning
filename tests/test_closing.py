...
def test_simple_close_closee_path(node_factory, bitcoind):
    ...
    'stored their tx' log), and that the peer's own closing tx is persisted.  Both
    ...
    #   "stored their tx" — handle_simpleclosed_their_closing_tx (peer's tx broadcast)
    #   "stored our tx" — handle_simpleclosed_our_closing_tx ran (our own tx stored)
    ...
    l1.daemon.wait_for_logs(['Simple close: stored our tx',
                             'Simple close: broadcast their tx'])
    l2.daemon.wait_for_logs(['Simple close: stored our tx',
                             'Simple close: broadcast their tx'])
