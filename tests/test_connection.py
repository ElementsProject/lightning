...
def test_open_channel2_tx_add_output_above_max_supply(node_factory, bitcoind):
    ...
    for sats in cases:
        ...
        # dualopend open_abort logs the MAX_MONEY failure reason. Empty PSBT,
        # so current total equals the rejected amount over 0 outputs.
        l1.daemon.wait_for_log(
            r"aborted open negotiation, tx-abort: "
            r"Output amount of {}sat exceeds max supply".format(sats)
        )
...
def test_open_channel2_tx_add_output_sum_above_max_supply(node_factory, bitcoind):
    """Two legal tx_add_output amounts that sum over MAX_MONEY must abort.

    Each output is individually <= MAX_MONEY, so libwally accepts them one by
    one; the running-sum check must still fail the negotiation before the
    combined total can crash dualopend on later PSBT extraction.
    """
    l1 = node_factory.get_node(options={"experimental-dual-fund": None})
    ...
    # 10M BTC + (11M BTC + 1 sat) == MAX_SUPPLY + 1.
    first_sats = 1000000000000000
    second_sats = 1100000000000001
    assert first_sats <= MAX_SUPPLY_SAT
    assert second_sats <= MAX_SUPPLY_SAT
    assert first_sats + second_sats == MAX_SUPPLY_SAT + 1
    ...
    mtype, payload = read_accept_channel2(lconn)
    assert mtype == WIRE_ACCEPT_CHANNEL2, (
        "open_channel2 rejected before interactive tx (got msgtype {})".format(mtype)
    )
    ...
    # First output is under the cap; dualopend with an empty local PSBT
    # should accept it and reply tx_complete.
    # serial_id even => from initiator.
    send_tx_add_output(lconn, channel_id, 0, first_sats, script)
    mtype, msg = read_tx_interactive_reply(lconn)
    assert mtype == WIRE_TX_COMPLETE, (
        "first tx_add_output sats={} was not accepted (got msgtype {})".format(
            first_sats, mtype
        )
    )

    # Second output is also under the cap alone, but the running sum exceeds
    # MAX_MONEY - must tx_abort rather than append and crash later.
    send_tx_add_output(lconn, channel_id, 2, second_sats, script)
    mtype, msg = read_tx_interactive_reply(lconn)
    assert mtype == WIRE_TX_ABORT, (
        "tx_add_output sum {}+{} was not aborted (got msgtype {})".format(
            first_sats, second_sats, mtype
        )
    )

    # dualopend open_abort logs the MAX_MONEY running-sum failure reason.
    l1.daemon.wait_for_log(
        r"aborted open negotiation, tx-abort: "
        r"Adding output amount {}sat would exceed max supply "
        r"\(current total is {}sat over 1 outputs\)".format(
            second_sats, first_sats + second_sats
        )
    )
