...
def test_plugin_command(node_factory):
    ...
    time.sleep(2)
    n.rpc.plugin(command="startdir", parameter=os.path.join(os.getcwd(), "contrib/plugins"))
    ...
    # Make sure 'reload', 'list', and 'unregister_hook' controls dont crash
    n.rpc.plugin(command="reload")
    n.rpc.plugin(command="list")
    n.rpc.plugin(command="unregister_hook", parameter="helloworld.py", second_parameter="htlc_accepted")
    time.sleep(1)

    # Make sure the plugin behaves normally after stop and restart
    n.rpc.plugin(command="stop", parameter="helloworld.py")
    n.daemon.wait_for_log(r"Killing plugin: helloworld.py")
    time.sleep(1)
    n.rpc.plugin(command="start", parameter=os.path.join(os.getcwd(), "contrib/plugins/helloworld.py"))
    n.daemon.wait_for_log(r"Plugin helloworld.py initialized")
