...
update-versions: update-pyln-versions update-wss-proxy-version update-reckless-version update-dot-version update-doc-examples
...
update-wss-proxy-version:
	@if [ -z "$(NEW_VERSION)" ]; then echo "Set NEW_VERSION!" >&2; exit 1; fi
	@echo "Updating plugins/wss-proxy to $(NEW_VERSION)"
	@sed -i '' 's/^version = .*/version = "$(NEW_VERSION)"/' plugins/wss-proxy/pyproject.toml
