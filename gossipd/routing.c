				...
				u32 index)
{
	...
	if (is_halfchan_defined(hc)) {
		...
		if (!ratelimit(rstate, &hc->tokens, hc->bcast.timestamp)) {
				   ...
				   u32 index)
{
	...
	if (node->bcast.index) {
		...
		/* Make sure it's not spamming us. */
		if (!ratelimit(rstate, &node->tokens, node->bcast.timestamp)) {
