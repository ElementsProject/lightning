...
static char completed, nested_completed;
...
static struct io_plan *actor_ready(struct io_conn *conn, void *unused)
{
	...
	if (!io_new_conn(NULL, replacement_fd, replacement_init, NULL))
		abort();
	/* A nested loop must not overwrite or release the outer loop's readiness
	 * snapshot. */
	assert(io_loop(NULL, NULL) == &nested_completed);
	return closed;
}
...
static struct io_plan *victim_ready(struct io_conn *conn, void *unused)
{
	io_break(&completed);
	return io_close(conn);
}
...
{
	io_plan_arg(conn, IO_IN);
	return io_set_plan(conn, IO_IN, victim_event, victim_ready, NULL);
}
