...
static bool test_transfer_crud(const tal_t *ctx)
{
	struct wallet_transfer t, *t2;
	struct wallet *w = create_test_wallet(ctx);
	...
	db_begin_transaction(w->db);
	CHECK(wallet_transfer_add(w, &t));
	CHECK(t.id != 0);
	t2 = wallet_transfer_by_payment_hash(ctx, w, &t.payment_hash);
	CHECK(t2 != NULL);
