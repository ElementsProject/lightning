...
static struct command_result *json_addpsbtutxo(struct command *cmd,
					       const char *buffer,
					       const jsmntok_t *obj UNNEEDED,
					       const jsmntok_t *params)
{
	struct json_stream *response;
	struct bitcoin_outpoint *utxo;
	struct wally_psbt *psbt;
	u32 *locktime;
	bool *add_initiator_serial_ids;
	...
	if (!param_check(cmd, buffer, params,
			 p_req("utxo", param_utxo, &utxo),
			 p_opt("initialpsbt", param_psbt, &psbt),
			 p_opt("locktime", param_number, &locktime),
			 p_opt_def("minconf", param_number, &minconf, 0),
			 p_opt_def("coinbase", param_bool, &coinbase, false),
			 p_opt_def("add_initiator_serial_ids", param_bool,
			 	   &add_initiator_serial_ids, false),
			 NULL))
}
