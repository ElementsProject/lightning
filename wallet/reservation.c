...
static const struct json_command addpsbtoutput_command = {
	...
	"bitcoin",
	json_addpsbtoutput,
	"Create a PSBT (or modify existing {initialpsbt}) with an output"
	" receiving {satoshi} amount.",
	false
};
						...
						const jsmntok_t *params)
{
	...
	if (!req_amount || amount_sat_zero(*req_amount))
...
static const struct json_command addpsbtinput_command = {
	...
	"bitcoin",
	json_addpsbtinput,
	"Create a PSBT (or modify existing {initialpsbt}) and add inputs that"
	" add up to at least {satoshi} amount (but typically more). You must"
	" return at least {emergency_msat} into the onchain wallet as change",
	false
};
