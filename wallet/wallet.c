...
void wallet_inflight_add(struct wallet *w, struct channel_inflight *inflight)
{
	...
	db_bind_txid(stmt, 1, &inflight->funding_txid);
	db_bind_int(stmt, 2, inflight->funding_outnum);
	db_bind_int(stmt, 3, inflight->funding_feerate);
	db_bind_amount_sat(stmt, 4, &inflight->funding);
	db_bind_amount_sat(stmt, 5, &inflight->our_funds);
			  ...
			  struct channel_inflight *inflight)
{
	...
	db_bind_txid(stmt, 2, &inflight->funding_txid);
	db_bind_int(stmt, 3, inflight->funding_outnum);
