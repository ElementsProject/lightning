...
use crate::{
    ...
    structs::{AppError, CheckRuneParams, PluginState},
    SWAGGER_FALLBACK,
};
...
pub async fn call_rpc_method(
    ...
    let (rest_map, mut rpc_params) = path_to_rest_map_and_params(&plugin, &path)?;
    ...
    if let Some(checkrune_params) = rest_map.rune {
