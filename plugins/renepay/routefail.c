...
static struct command_result *handle_cases_cb(struct routefail *r)
{
	...
	default:
		// TODO: remember you might be disabling the same channel
		// multiple times
		payment_disable_chan(route->payment, *result->erring_channel,
...
void *routefail_virtual_program[] = {
    &update_gossip_routefail_mod, &update_knowledge_routefail_mod,
    &handle_cases_routefail_mod, &end_routefail_mod, NULL};
