    ...
    if let Some(domain) = url.domain() {
        if domain == "github.com" {
            if let Some(github_redir) = &plugin.state().github_redir {
                let line = format!("using github redirect: file://{github_redir}");
                logger.log(&line, LogLevel::INFO).await?;
                url = Url::from_str(&format!("file://{github_redir}"))?;
            }
        }
    }
...
pub async fn search_sources(
    ...
    for url_str in &urls {
        ...
        let repo_dir = init_plugin_repo(plugin, url.clone(), logger).await?;
        rl_plugins.extend(
            find_plugin_locs(
