...
use crate::{
    structs::{
        CommonResult, DisableResponse, EnableArgs, EnableResponse, PluginState, RecklessLogger,
        RecklessPlugin, RecklessTopic,
    },
    util::{
...
pub async fn handle_enable(
    ...
    let response = EnableResponse::new(common_result, logger);
...
pub async fn handle_disable(
    ...
    let common_result = match disable_plugin(plugin.clone(), &rl_plugin, &mut logger).await {
        Ok(_) => CommonResult { plugin_name },
        Err(e) => {
    ...
    let response = DisableResponse::new(common_result, logger);
