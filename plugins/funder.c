...
static struct pending_open *
new_channel_open(const tal_t *ctx, struct plugin *p, const struct node_id id,
...
static struct command_result *unreserve_done(struct command *cmd UNUSED,
					     const char *buf,
...
static void unreserve_psbt(struct pending_open *open)
{
	...
	plugin_log(
	    open->p, LOG_DBG, "Calling `unreserveinputs` for channel %s",
	    type_to_string(tmpctx, struct channel_id, &open->channel_id));

	req = jsonrpc_request_start(open->p, NULL, "unreserveinputs",
				    unreserve_done, unreserve_done, open);
...
static struct command_result *command_hook_cont_psbt(struct command *cmd,
					    ...
					    struct pending_open *open)
{
	...
	plugin_log(
	    cmd->plugin, LOG_DBG, "`signpsbt` done for channel %s",
	    type_to_string(tmpctx, struct channel_id, &open->channel_id));
	err = json_scan(tmpctx, buf, result, "{signed_psbt:%}",
			JSON_SCAN_TAL(cmd, json_to_psbt, &signed_psbt));

	if (err)
		plugin_err(
		    cmd->plugin, "`signpsbt` payload did not scan %s: %*.s",
		    err, json_tok_full_len(result), json_tok_full(buf, result));
...
json_openchannel2_sign_call(struct command *cmd, const char *buf,
			    const jsmntok_t *params)
{
	...
	plugin_log(cmd->plugin, LOG_DBG, "openchannel_sign PSBT is %s",
		   type_to_string(tmpctx, struct wally_psbt, psbt));

	req = jsonrpc_request_start(cmd->plugin, cmd, "signpsbt",
				    &signpsbt_done, &forward_error, open);
...
static struct command_result *
json_openchannel2_changed_call(struct command *cmd, const char *buf,
			       const jsmntok_t *params)
{
	struct channel_id cid;
	struct wally_psbt *psbt;
	...
	plugin_log(cmd->plugin, LOG_DBG, "openchannel_changed PSBT is %s",
		   type_to_string(tmpctx, struct wally_psbt, psbt));
...
static struct command_result *psbt_funded(struct command *cmd, const char *buf,
					  const jsmntok_t *result,
					  struct open_info *info)
{
	...
	err = json_scan(tmpctx, buf, result, "{psbt:%}",
			JSON_SCAN_TAL(tmpctx, json_to_psbt, &psbt));
	if (err)
		plugin_err(
		    cmd->plugin, "`fundpsbt` response did not scan %s: %.*s",
		    err, json_tok_full_len(result), json_tok_full(buf, result));
	...
	new_channel_open(cmd->plugin, cmd->plugin, info->id, info->cid, psbt);
...
static struct command_result *psbt_fund_failed(struct command *cmd,
					       const char *buf,
...
static struct command_result *
param_msat_as_sat(struct command *cmd, const char *name, const char *buffer,
		  const jsmntok_t *tok, struct amount_sat **sat)
{
	struct amount_msat msat;

	*sat = tal(cmd, struct amount_sat);
	if (parse_amount_msat(&msat, buffer + tok->start,
			      tok->end - tok->start) &&
...
static struct command_result *listfunds_success(struct command *cmd,
						const char *buf,
						const jsmntok_t *result,
						struct open_info *info)
{
	...
	funding_err = calculate_our_funding(
	    current_policy, info->id, info->their_funding, available_funds,
	    info->channel_max, info->requested_lease, &info->our_funding);
	plugin_log(
	    cmd->plugin, LOG_DBG, "Policy %s returned funding amount of %s. %s",
	...
	plugin_log(
	    cmd->plugin, LOG_DBG, "Funding channel %s with %s (their input %s)",
	    type_to_string(tmpctx, struct channel_id, &info->cid),
	    type_to_string(tmpctx, struct amount_sat, &info->our_funding),
	    type_to_string(tmpctx, struct amount_sat, &info->their_funding));

	req = jsonrpc_request_start(cmd->plugin, cmd, "fundpsbt", &psbt_funded,
				    &psbt_fund_failed, info);
	json_add_string(
	    req->js, "satoshi",
	    type_to_string(tmpctx, struct amount_sat, &info->our_funding));
...
static struct command_result *listfunds_failed(struct command *cmd,
					       const char *buf,
					       const jsmntok_t *error,
					       struct open_info *info)
{
...
static struct command_result *json_openchannel2_call(struct command *cmd,
						     const char *buf,
						     const jsmntok_t *params)
{
	struct open_info *info = tal(cmd, struct open_info);
	struct amount_msat max_htlc_inflight, htlc_minimum;
	u64 commitment_feerate_perkw, feerate_our_max, feerate_our_min;
	...
	if (err)
		plugin_err(
		    cmd->plugin, "`openchannel2` payload did not scan %s: %.*s",
		    err, json_tok_full_len(params), json_tok_full(buf, params));
	...
	if (info->funding_feerate_perkw < feerate_our_min ||
		...
		plugin_log(cmd->plugin, LOG_DBG,
			   ...
			   info->funding_feerate_perkw, feerate_our_min,
			   feerate_our_max);
	...
	req =
	    jsonrpc_request_start(cmd->plugin, cmd, "listfunds",
				  &listfunds_success, &listfunds_failed, info);

	return send_outreq(cmd->plugin, req);
}

/* Peer has asked us to RBF */
static struct command_result *json_rbf_channel_call(struct command *cmd,
						    const char *buf,
						    const jsmntok_t *params)
{
	...
	if (err)
		plugin_err(
		    cmd->plugin, "`rbf_channel` payload did not scan %s: %.*s",
		    err, json_tok_full_len(params), json_tok_full(buf, params));

	/* If there's no channel_max, it's actually infinity */
	err =
	    json_scan(tmpctx, buf, params, "{rbf_channel:{channel_max_msat:%}}",
		      JSON_SCAN(json_to_msat_as_sats, &info->channel_max));
	if (err)
		info->channel_max = AMOUNT_SAT(UINT64_MAX);
...
static struct command_result *
json_disconnect(struct command *cmd, const char *buf, const jsmntok_t *params)
{
	struct node_id id;
	const char *err;

	err = json_scan(tmpctx, buf, params, "{id:%}",
	...
	plugin_log(cmd->plugin, LOG_DBG, "Cleaning up inflights for peer id %s",
			    ...
			    struct funder_policy *policy)
{
	...
	json_add_string(stream, "policy", funder_opt_name(policy->opt));
...
static struct command_result *param_whitelist_subcommand(struct command *cmd,
							 const char *name,
							 const char *buffer,
							 const jsmntok_t *tok,
							 bool **is_add)
{
	...
	subcommand_str =
	    tal_strndup(cmd, buffer + tok->start, tok->end - tok->start);
	/* We expect the subcommand to be one of 'add' or 'remove' */
	*is_add = tal(cmd, bool);
	if (streq(subcommand_str, "add")) {
		**is_add = true;
		return NULL;
	} else if (streq(subcommand_str, "remove")) {
		**is_add = false;
		return NULL;
	}
	...
	tal_free(*is_add);
	return command_fail_badparam(cmd, name, buffer, tok,
				     "Subcommand options: 'add' 'remove'");
}

static struct command_result *
param_whitelist_pubkeys(struct command *cmd, const char *name, 
			const char *buffer, const jsmntok_t *tok,
			struct node_id **nodeids)
{
	size_t i;
	const jsmntok_t *json_pubkey;
	...
	*nodeids = tal_arr(cmd, struct node_id, tok->size);

	json_for_each_arr(i, json_pubkey, tok) {
		struct command_result *res;
		res = param_node_id(cmd, NULL, buffer, json_pubkey, 
				    &nodeids[i]);
		if (res)
			return res;
	}
...
static struct command_result *
param_funder_opt(struct command *cmd, const char *name, const char *buffer,
		 const jsmntok_t *tok, enum funder_opt **opt)
{
	char *opt_str, *err;

	*opt = tal(cmd, enum funder_opt);
	opt_str = tal_strndup(cmd, buffer + tok->start, tok->end - tok->start);
...
static struct command_result *param_policy_mod(struct command *cmd,
					       const char *name,
					       const char *buffer,
					       const jsmntok_t *tok, u64 **mod)
{
	struct amount_sat sats;
	char *arg_str, *err;

	*mod = tal(cmd, u64);
	arg_str = tal_strndup(cmd, buffer + tok->start, tok->end - tok->start);
...
static struct command_result *
parse_lease_rates(struct command *cmd, const char *buffer, const jsmntok_t *tok,
		  struct funder_policy *policy,
		  struct funder_policy *current_policy, u32 *lease_fee_basis,
		  struct amount_sat *lease_fee_sats, u32 *funding_weight,
...

{
	...
	if (!lease_rates_empty(current_policy->rates))
		policy->rates =
		    tal_dup(policy, struct lease_rates, current_policy->rates);
	else if (lease_fee_basis || lease_fee_sats || funding_weight ||
		 channel_fee_max_proportional_thousandths || chan_fee_msats)
...
#define ASSIGN_OR_RETURN_FAIL(type, member)                                    \
	do {                                                                   \
		if (member &&                                                  \
			...
			return command_fail_badparam(cmd, #member, buffer,     \
						     tok, "overflow");         \
	} while (0)
...
#undef ASSIGN_OR_RETURN_FAIL
	...
	if (lease_fee_sats &&
		...
		return command_fail_badparam(cmd, "lease_fee_base_sat", buffer,
					     tok, "overflow");
	}
...
static struct command_result *leaserates_set(struct command *cmd,
					     const char *buf,
					     const jsmntok_t *result,
					     struct funder_policy *policy)
{
...
static struct command_result *json_funder_whitelist(struct command *cmd,
						    const char *buf,
						    const jsmntok_t *params)
{
	bool *is_add;
	...
	if (!param(cmd, buf, params,
		   ...
		   p_opt("pubkeys", param_whitelist_pubkeys, &nodeids),
		   NULL))
...
static struct command_result *
json_funderupdate(struct command *cmd, const char *buf, const jsmntok_t *params)
{
	struct amount_sat *min_their_funding, *max_their_funding,
	    *per_channel_min, *per_channel_max, *reserve_tank, *lease_fee_sats;
	struct amount_msat *channel_fee_msats;
	u32 *fuzz_factor, *fund_probability, *chan_fee_ppt, *lease_fee_basis,
	    *funding_weight;
	u64 *mod;
	bool *leases_only;
	enum funder_opt *opt;
	...
	if (!param(cmd, buf, params,
		   ...
		   p_opt_def("per_channel_min_msat", param_msat_as_sat,
			     &per_channel_min, current_policy->per_channel_min),
		   p_opt_def("per_channel_max_msat", param_msat_as_sat,
			     &per_channel_max, current_policy->per_channel_max),
		   p_opt_def("reserve_tank_msat", param_msat_as_sat,
			     &reserve_tank, current_policy->reserve_tank),
		   p_opt_def("fuzz_percent", param_number, &fuzz_factor,
			     current_policy->fuzz_factor),
		   p_opt_def("fund_probability", param_number,
			     &fund_probability,
			     current_policy->fund_probability),
		   p_opt("lease_fee_base_msat", param_msat_as_sat,
			 &lease_fee_sats),
	...
	res = parse_lease_rates(cmd, buf, params, policy, current_policy,
				lease_fee_basis, lease_fee_sats, funding_weight,
				chan_fee_ppt, channel_fee_msats);
	if (res)
		return res;
	...
	if (err) {
		tal_free(policy);
		return command_done_err(cmd, JSONRPC2_INVALID_PARAMS, err,
					NULL);
	}
	...
	req = jsonrpc_request_start(cmd->plugin, cmd, "setleaserates",
...
static const struct plugin_command commands[] = {
    {"funderupdate", "liquidity", "Configuration for dual-funding settings.",
     "Update current settings. Modifies how node reacts to"
     " incoming channel open requests. Responds with list"
     " of current configs.",
     json_funderupdate},
    {"funder-whitelist", "liquidity", "Add or remove funder whitelisted nodes",
     "Update current settings. Modifies how node reacts to"
     " incoming channel open requests. Responds with list"
     " of current configs.",
					...
					struct lease_rates *rates)
{
	...
	json_out_add(jout, "lease_fee_basis", false, "%d",
		     rates->lease_fee_basis);

	json_out_add(jout, "funding_weight", false, "%d",
	...
	rpc_scan(p, "setleaserates", take(jout),
		 /* Unused */
		 "{lease_fee_base_msat:%}", JSON_SCAN(json_to_msat, &mval));
}
...
static void memleak_mark(struct plugin *p, struct htable *memtable)
{
	...
	memleak_remove_region(memtable, current_policy,
			      sizeof(*current_policy));
}
#endif
						...
						struct funder_policy *policy)
{
	if (!policy->rates)
		policy->rates = default_lease_rates(policy);
	return u16_option(
	    arg, &policy->rates->channel_fee_max_proportional_thousandths);
}
...
static char *amount_sat_or_u64_option(const char *arg, u64 *amt)
{
	...
	if (err) {
		...
		if (!parse_amount_sat(&sats, arg, strlen(arg)))
			return tal_fmt(NULL, "Unable to parse option '%s'",
				       arg);
...
static char *option_funder_node_whitelist(const char *arg,
					  struct funder_policy *policy)
{
	if (!policy->rates)
		policy->rates = default_lease_rates(policy);
...
int main(int argc, char **argv)
{
	...
	plugin_main(
	    argv, init, PLUGIN_RESTARTABLE, true, NULL, commands,
	    ARRAY_SIZE(commands), notifs, ARRAY_SIZE(notifs), hooks,
	    ARRAY_SIZE(hooks), NULL, 0,
	    plugin_option("funder-policy", "string",
			  "Policy to use for dual-funding requests."
			  " [match, available, fixed]",
			  funding_option, &current_policy->opt),
	    plugin_option("funder-policy-mod", "string",
			  "Percent to apply policy at"
			  " (match/available); or amount to fund"
			  " (fixed)",
			  amount_sat_or_u64_option, &current_policy->mod),
	    plugin_option("funder-min-their-funding", "string",
			  "Minimum funding peer must open with"
			  " to activate our policy",
			  amount_option, &current_policy->min_their_funding),
	    plugin_option("funder-max-their-funding", "string",
			  "Maximum funding peer may open with"
			  " to activate our policy",
			  amount_option, &current_policy->max_their_funding),
	    plugin_option("funder-per-channel-min", "string",
			  "Minimum funding we'll add to a channel."
			  " If we can't meet this, we don't fund",
			  amount_option, &current_policy->per_channel_min),
	    plugin_option("funder-per-channel-max", "string",
			  "Maximum funding we'll add to a channel."
			  " We cap all contributions to this",
			  amount_option, &current_policy->per_channel_max),
	    plugin_option("funder-reserve-tank", "string",
			  "Amount of funds we'll always leave"
			  " available.",
			  amount_option, &current_policy->reserve_tank),
	    plugin_option("funder-fuzz-percent", "int",
			  "Percent to fuzz the policy contribution by."
			  " Defaults to 0%. Max is 100%",
			  u32_option, &current_policy->fuzz_factor),
	    plugin_option("funder-fund-probability", "int",
			  "Percent of requests to consider."
			  " Defaults to 100%. Setting to 0% will"
			  " disable dual-funding",
			  u32_option, &current_policy->fund_probability),
	    plugin_option("funder-lease-requests-only", "bool",
			  "Only fund lease requests. Defaults to"
			  " true if channel lease rates are"
			  " being advertised",
			  bool_option, &current_policy->leases_only),
	    plugin_option("lease-fee-base-sat", "string",
			  "Channel lease rates, base fee for leased"
			  " funds, in satoshi.",
			  option_lease_fee_base, current_policy),
	    plugin_option_deprecated("lease-fee-base-msat", "string",
				     "Channel lease rates, base fee for leased"
				     " funds, in satoshi.",
				     option_lease_fee_base, current_policy),
	    plugin_option("lease-fee-basis", "int",
			  "Channel lease rates, basis charged"
			  " for leased funds (per 10,000 satoshi.)",
			  option_lease_fee_basis, current_policy),
	    plugin_option("lease-funding-weight", "int",
			  "Channel lease rates, weight"
			  " we'll ask opening peer to pay for in"
			  " funding transaction",
			  option_lease_weight_max, current_policy),
	    plugin_option("channel-fee-max-base-msat", "string",
	    ...
	    plugin_option("channel-fee-max-proportional-thousandths", "int",
