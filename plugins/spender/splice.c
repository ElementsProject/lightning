			...
			struct splice_index_pkg *pkg)
{
	...
	plugin_log(cmd->plugin, LOG_DBG, "addpsbt_get_result(%s)",
		   fmt_wally_psbt(tmpctx, splice_cmd->psbt));
					  ...
					  size_t index)
{
	...
	if (!amount_sat_is_zero(action->in_sat)) {
		...
		plugin_log(cmd->plugin, LOG_DBG, "splice_init(+%s) index: %d",
			   fmt_amount_sat(tmpctx, action->in_sat),
			   (int)index);
		...
		plugin_log(cmd->plugin, LOG_DBG, "splice_init(0) index: %d",
			   (int)index);
	}
