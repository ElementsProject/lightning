					      ...
					      struct invreq *ir)
{
	...
	if (!best) {
		...
		if (ir->inv->invoice_relative_expiry)
			base = od->blockheight + 6 + *ir->inv->invoice_relative_expiry / 600;
		else
			base = od->blockheight + 6 + 7200 / 600;

		etlvs[0]->payment_constraints = tal(etlvs[0],
						    struct tlv_encrypted_data_tlv_payment_constraints);
		etlvs[0]->payment_constraints->max_cltv_expiry = base + best->cltv + od->cltv_final;
